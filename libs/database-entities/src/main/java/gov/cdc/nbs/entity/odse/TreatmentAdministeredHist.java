...
@Setter
@Entity
@Table(name = "Treatment_administered_hist")
public class TreatmentAdministeredHist {
    @EmbeddedId
    private TreatmentAdministeredHistId id;

    @MapsId
    @ManyToOne(fetch = FetchType.LAZY, optional = false)
    @JoinColumn(name = "treatment_uid", referencedColumnName = "treatment_uid", nullable = false)
    @JoinColumn(name = "treatment_administered_seq", referencedColumnName = "treatment_administered_seq",
            nullable = false)
    ...
    @Column(name = "rate_unit_cd", length = 20)
    private String rateUnitCd;
