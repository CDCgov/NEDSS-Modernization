...
@Component
@ScenarioScope
class PlaceOptionMother {

  private static final String DELETE_IN =
      """
      delete from Place where place_uid in (:identifiers);
  ...
  private static final String CREATE =
      """
        insert into Entity(entity_uid, class_cd) values (:identifier, 'PLC');
        insert into Entity_id(entity_uid, entity_id_seq, type_cd, root_extension_txt)
            values (:identifier, 1, :type_cd, :identifier);
        insert into Place(place_uid, nm) values (:identifier, :name);
          """;
  ...
  PlaceOptionMother(final JdbcTemplate template) {
