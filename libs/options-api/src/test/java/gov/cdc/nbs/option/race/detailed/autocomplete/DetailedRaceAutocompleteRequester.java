...
class DetailedRaceAutocompleteRequester {
  ...
  ResultActions complete(final String category, final String criteria) throws Exception {
    return mvc.perform(
        get("/nbs/api/options/races/{category}/search", category).param("criteria", criteria));
  }

  ResultActions complete(final String category, final String criteria, final int limit)
      throws Exception {
    return mvc.perform(
