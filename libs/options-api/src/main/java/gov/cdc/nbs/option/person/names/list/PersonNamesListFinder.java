...
public class PersonNamesListFinder {
  ...
  Collection<Option> find() {
    ...
    String programAreas =
        stdHivProgramAreas.stream().map(item -> "'" + item + "'").collect(Collectors.joining(", "));

    return this.client.sql(QUERY.formatted(programAreas)).query(this.mapper).list();
  }
}
