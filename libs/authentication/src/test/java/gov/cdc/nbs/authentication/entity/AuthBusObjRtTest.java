...
class AuthBusObjRtTest {
    ...
    void should_initialize_with_audit_from_set() {
        ...
        AuthPermSet set = new AuthPermSet()
            .audit(
                new AuthAudit(
                    new AuthAudit(
                        1139L,
                        Instant.parse("2023-07-14T01:02:03Z")
                    )
                )
            );
        ...
        AuthBusObjRt actual = new AuthBusObjRt(
            set,
            objectType,
            operationType
        );
    ...
    void should_initialize_with_created_operation_right() {
        ...
        AuthBusObjRt objectRight = new AuthBusObjRt(
            set,
            objectType,
            operationType
        );
        ...
        assertThat(objectRight.operationRight())
            .satisfies(
                actual -> assertThat(actual.objectRight()).isSameAs(objectRight)
            )
            .satisfies(
                actual -> assertThat(actual.operationType()).isSameAs(operationType)
            );
