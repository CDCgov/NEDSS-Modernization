...
@Component
class UserPermissionFinder {
  private final JPAQueryFactory queryFactory;

  UserPermissionFinder(
      @Qualifier("authentication") final JPAQueryFactory queryFactory
  ) {
    this.queryFactory = queryFactory;
  }

  /**
   * Queries the database for a list of user permissions For each distinct permission, create a {@link NbsAuthority} if
   ...
   */
  Set<NbsAuthority> getUserPermissions(AuthUser user) {
  ...
  private NbsAuthority buildNbsAuthority(Tuple t) {
    ...
    if ((userIsGuest.equals('F') && userRight != null && userRight.equals('T'))
      ...
      return new NbsAuthority(
          businessOperation,
          businessObject,
          progArea,
          businessOperation + "-" + businessObject
      );
    } else {
      return null;
    }
