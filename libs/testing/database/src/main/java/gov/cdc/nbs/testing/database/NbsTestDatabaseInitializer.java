...
class NbsTestDatabaseInitializer
  ...
  public void initialize(final ConfigurableApplicationContext context) {
    ...
    loadBaselineConfiguration(container);
  ...
  /**
   * Directly establishes a connection to the starting container and loads the required global
   * configuration rows if they are not already present.
   */
  private void loadBaselineConfiguration(final JdbcDatabaseContainer<?> container) {
    ...
    try (Connection conn = container.createConnection("");
      ...
      throw new IllegalStateException("Failed to load baseline database configurations", e);
    }
  }
}
