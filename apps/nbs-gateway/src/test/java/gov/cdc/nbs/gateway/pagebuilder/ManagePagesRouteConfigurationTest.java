...
class ManagePagesRouteConfigurationTest {
  ...
  void should_contain_route() {
    ...
    assertTrue(
        pagebuilderManagePagesConfig
            ...
            .any(
                i ->
                    i.getId().equals("pagebuilder-manage-pages")
                        && i.getUri().equals(service.uri()))
            .block());
  }
  ...
  void should_route_to_modernized() {
    ...
    pagebuilderApi.stubFor(
        get(urlPathMatching("/nbs/page-builder/redirect"))
  ...
  void should_not_route_without_method_param() {
    ...
    webClient
        .get()
        .uri(builder -> builder.path("/nbs/ManagePage.do").queryParam("initLoad", "true").build())
        .exchange()
        .expectStatus()
        .isNotFound();
  }
