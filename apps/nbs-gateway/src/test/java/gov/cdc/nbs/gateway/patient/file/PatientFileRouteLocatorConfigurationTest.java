...
class PatientFileRouteLocatorConfigurationTest {
  ...
  void should_route_to(final String action) {
    service.stubFor(
        get(urlPathMatching("/nbs/redirect/patient/file/summary/return\\\\?.*")).willReturn(ok()));

    webClient
        .get()
        .uri(builder -> builder.path("/nbs/any/path").queryParam("ContextAction", action).build())
        .exchange()
        .expectStatus()
        .isOk();
  }
  ...
  void should_route_directly_to_the_patient_profile(final String action, final String type) {
    service.stubFor(get(urlPathMatching("/nbs/redirect/patient/file")).willReturn(ok()));
