...
class NBS6LoginRouteLocatorConfigurationTest {
  ...
  void should_bypass_NBS6_login() {
    ...
    WebTestClient
        ...
        .configureClient()
        .build()
        .mutateWith(SecurityMockServerConfigurers.mockUser("authenticated-user")).get().uri(
            builder -> builder.path("/nbs/login")
                .build()
        ).exchange()
        .expectStatus()
        .isOk();
  ...
  void should_block_NBS6_nfc_based_login() {
    webClient
        .get().uri(
            builder -> builder
                .path("/nbs/nfc")
                .queryParam("UserName", "anything")
                .build()
        )
        .exchange()
        .expectStatus().is3xxRedirection()
        .expectHeader().location(home.base());
  }
