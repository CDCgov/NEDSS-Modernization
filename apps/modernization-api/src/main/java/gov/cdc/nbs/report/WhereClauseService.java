...
public class WhereClauseService {
    ...
    /**
     * Constructs WHERE clause based on the basic filters for a report.
     * Each basic filter in the configuration is processed and joined with AND logic.
     * @param reportConfig is the full configuration for the report. Most relevant here are the columns and active filters.
     * @return A formatted SQL string starting with WHERE. An empty string is returned if required parts are empty
     */
    public String buildBasicWhereClause(ReportConfiguration reportConfig) {
        ...
        for (FilterConfiguration filter : reportConfig.filters()) {
            findColumn(reportConfig, filter).ifPresent(column -> {
     ...
     */
    private Optional<ReportColumn> findColumn(ReportConfiguration reportConfig, FilterConfiguration filterConfig) {
