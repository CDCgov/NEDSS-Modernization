...
public class SearchSorting {
  ...
  public static SortOptions asSortOption(final String name, final SortOrder order) {
    return SortOptions.of(sort -> sort.field(field -> field.field(name).order(order)));
  }
  ...
  public static SortOptions asHandlingNullSortOption(final String name, final SortOrder order) {
    return SortOptions.of(
        sort -> sort.field(field -> field.field(name).order(order).missing(resolveMissing(order))));
  }

  public static SortOptions asSortOption(
      final String path, final String name, final SortOrder order) {
    return SortOptions.of(
        sort ->
            sort.field(
                field ->
            ...
            sort.field(
                field ->
                    field
                        .field(name)
                        .order(order)
                        .nested(nested -> nested.path(path).filter(filter))));
  }
