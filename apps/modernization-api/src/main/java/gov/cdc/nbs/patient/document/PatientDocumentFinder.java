...
class PatientDocumentFinder {
  ...
  Page<PatientDocument> find(
    ...
    long total = resolveTotal(patient, scope);
    return total > 0
        ? new PageImpl<>(resolvePage(patient, pageable, scope), pageable, total)
        : Page.empty(pageable);
  }
  ...
  private <R> JPAQuery<R> applyBaseCriteria(
    ...
    return query.from(PATIENT)
        ...
        .join(DOCUMENT).on(
            ...
            DOCUMENT.programJurisdictionOid.in(scope.any())
        )
  ...
  private <R> JPAQuery<R> applyCriteria(
    ...
    return applyBaseCriteria(query, patient, scope)
  ...
  private long resolveTotal(final long patient, final PermissionScope scope) {
    Long total = applyBaseCriteria(
