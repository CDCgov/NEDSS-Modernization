 ...
 * {@link Optional}. When there is more than one row present a merge function is used to combine the
 * values into one instance. An empty {@code Optional} is returned when the {@code ResultSet} is
 * empty or the mapper resolves a {@code null} value.
 *
...
public class MergingResultSetExtractor<V> implements ResultSetExtractor<Optional<V>> {
   ...
   */
  public MergingResultSetExtractor(final RowMapper<V> mapper, final BinaryOperator<V> merger) {
