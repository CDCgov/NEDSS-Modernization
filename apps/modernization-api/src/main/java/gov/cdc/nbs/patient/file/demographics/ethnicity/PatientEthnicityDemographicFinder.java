...
class PatientEthnicityDemographicFinder {
  ...
  PatientEthnicityDemographicFinder(final JdbcClient client) {
    this.client = client;
    this.extractor =
        new MergingResultSetExtractor<>(
            new PatientEthnicityDemographicRowMapper(), PatientEthnicityDemographicMerger::merge);
  }

  Optional<PatientEthnicityDemographic> find(final long patient) {
    return this.client.sql(QUERY).param(patient).query(this.extractor);
  }
}
