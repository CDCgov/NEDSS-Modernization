...
public class AssociatedInvestigationFinder {
  ...
  public AssociatedInvestigationFinder(final JdbcClient client) {
    ...
    this.extractor = new MultiMapResultSetExtractor<>(
        ...
        new AssociatedInvestigationRowMapper(new AssociatedInvestigationRowMapper.Column(2, 3, 4, 5))
    );
  }

  public Map<Long, Collection<AssociatedInvestigation>> find(
      final Collection<Long> sources,
      final PermissionScope scope
  ) {
    return !scope.allowed() || sources.isEmpty()
        ? Collections.emptyMap()
        : execute(sources, scope);
  }

  Map<Long, Collection<AssociatedInvestigation>> execute(final Collection<Long> sources, final PermissionScope scope) {
