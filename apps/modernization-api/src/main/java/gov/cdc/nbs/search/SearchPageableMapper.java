...
public class SearchPageableMapper {
  ...
  public SearchPageableMapper(@Value("${nbs.search.max-page-size}") final int maxPageSize) {
    this.maxPageSize = maxPageSize;
  }

  public Pageable from(final PaginationRequest request) {
    return request == null ? PageRequest.of(0, maxPageSize) : asPageable(request);
  }

  private Pageable asPageable(final PaginationRequest request) {
    if (request.pageSize() != null && request.pageSize() > maxPageSize) {
      throw new IllegalArgumentException(
          "Invalid page size: " + request.pageSize() + ". Max size allowed: " + maxPageSize);
    }
