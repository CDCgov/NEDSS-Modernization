...
class PatientRaceDemographicMerger {
  ...
  static PatientRaceDemographic merge(
      final PatientRaceDemographic current, final PatientRaceDemographic next) {

    Collection<Selectable> detailed = CollectionMerge.merged(current.detailed(), next.detailed());

    return new PatientRaceDemographic(current.asOf(), current.race(), detailed);
  }
