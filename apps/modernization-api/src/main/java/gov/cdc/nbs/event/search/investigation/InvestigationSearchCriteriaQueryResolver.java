...
class InvestigationSearchCriteriaQueryResolver {
                ...
                .query(
                    query -> query.bool(
                        bool -> bool.filter(
                            ...
                            .must(
                                must -> must.prefix(
                                    prefix -> prefix.field(SearchableInvestigation.IDENTIFICATION_VALUE)
  ...
  private Optional<QueryVariant> withCaseIdentifier(final InvestigationFilter criteria) {
    return criteria.caseId().map(
        identifier -> MatchQuery.of(
            match -> match.field("local_id").query(identifier.getId())
        )
    );
  }

  private Optional<QueryVariant> withNotification(InvestigationFilter criteria) {
    return criteria.notification().map(
        identifier -> MatchQuery.of(
