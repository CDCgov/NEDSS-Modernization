...
class PatientDemographicsSummaryResolver {
  ...
  /**
   * Resolves the {@link PatientDemographicsSummary} for a patient based on the given {@code asOf} if the patient can be
   * found.
   *
   * @param patient The unique identifier of a patient
   * @param asOf    The date to find the most recent demographics for
   * @return An {@code Optional} containing the {@code PatientDemographicsSummary} of the patient for the given
   * {@code asOf} or empty if the patient cannot be found.
   */
  ...
  private PatientDemographicsSummary resolveMultiValueDemographics(
      final SummarizedPatientDemographics summarized,
      final long patient,
    ...
    Collection<DisplayableIdentification> identifications = this.identificationFinder.find(patient, asOf);
