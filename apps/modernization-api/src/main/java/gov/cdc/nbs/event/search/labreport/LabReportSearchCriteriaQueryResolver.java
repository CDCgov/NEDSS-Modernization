...
class LabReportSearchCriteriaQueryResolver {
  ...
  private Optional<QueryVariant> withLabIdentifier(final LabReportFilter criteria) {
    return criteria
        ...
        .map(
            identifier ->
                MatchQuery.of(match -> match.field("local_id").query(identifier.getLabEventId())));
  }
                            ...
                            .query(
                                query ->
                                    query.simpleQueryString(
                                        simple ->
                                            simple
                                                .fields("observations.cd_desc_txt")
                            ...
                            .query(
                                query ->
                                    query.simpleQueryString(
                                        simple ->
                                            simple
                                                .fields("observations.display_name")
                                                .query(WildCards.startsWith(value))
                                                .defaultOperator(Operator.And)))));
  }
}
