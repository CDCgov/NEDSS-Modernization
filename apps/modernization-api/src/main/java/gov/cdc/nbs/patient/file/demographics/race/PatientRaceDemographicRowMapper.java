...
class PatientRaceDemographicRowMapper implements RowMapper<PatientRaceDemographic> {
  ...
  record Column(int asOf, SelectableRowMapper.Column category, SelectableRowMapper.Column detail) {
    Column() {
      this(1, new SelectableRowMapper.Column(2, 3), new SelectableRowMapper.Column(4, 5));
    }
  }
  ...
  public PatientRaceDemographic mapRow(final ResultSet resultSet, int rowNum) throws SQLException {
    ...
    List<Selectable> detailed =
        detail == null || Objects.equals(detail.value(), category.value())
            ? List.of()
            : List.of(detail);

    return new PatientRaceDemographic(asOf, category, detailed);
  }
}
