...
public class InvestigationNotificationSteps {
  ...
  public String notificationStatus(final String value) {
    return resolver.resolve("REC_STAT_NOT_UI", value).orElse(null);
  }
  ...
  public void the_investigation_was_notified_of(final String value, final Instant on) {
    if (value != null) {
      this.activeInvestigation
          .maybeActive()
          .ifPresent(investigation -> mother.create(investigation, value, on));
    }
  }
  ...
  public void the_investigation_has_a_notification_status_of(final String value) {
    if (value != null) {
      this.activeInvestigation
          .maybeActive()
          .ifPresent(investigation -> mother.create(investigation, value, Instant.now()));
    }
  }

  @Given(
      "the notification exists in the CN_TransportQ_out table with status of {notificationTransportStatus}")
  public void the_notification_exists_in_the_transportq_out_table_with_status_of(
      final String status) {
    this.activeInvestigation
        .maybeActive()
        .ifPresent(investigation -> mother.createTransportStatus(status));
  }
  ...
  public void i_receive_a_notification_transport_status(final String status) throws Exception {
    response.active().andExpect(jsonPath("$.status").value(equalTo(status)));
  }
