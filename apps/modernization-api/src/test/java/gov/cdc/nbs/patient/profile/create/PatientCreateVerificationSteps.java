...
public class PatientCreateVerificationSteps {
  ...
  public void the_new_patient_identifier_is_returned() throws Exception {
    this.response.active().andExpect(jsonPath("$.id").exists());
  }
  ...
  public void the_new_patient_local_identifier_is_returned() throws Exception {
    this.response.active().andExpect(jsonPath("$.local").exists());
  }
  ...
  public void the_new_patient_short_identifier_is_returned() throws Exception {
    this.response.active().andExpect(jsonPath("$.shortId").exists());
  }

  @Then("the new patient's name {string} {string} is returned")
  public void the_new_patient_name_is_returned(final String first, final String last)
      throws Exception {
    this.response
        .active()
        .andExpectAll(jsonPath("$.name.first").value(first), jsonPath("$.name.last").value(last));
  }
}
