...
public class InvestigationSearchSteps {
  ...
  private List<Investigation> results;
  private final GraphQLPage page = new GraphQLPage(25);
  ...
  private List<Investigation> search(final InvestigationFilter filter) {
    return authenticated.perform(() -> investigationResolver.findInvestigationsByFilter(filter, page).getContent());
  }
  ...
  public void i_find_the_investigations_with_field_and_status(final String field, final String statusString) {
    ...
    assertThat(results).allSatisfy(assertion);
  }
