...
public class NBS6PatientFileSteps {
  ...
  public void i_navigate_to_the_nbs6_patient_file() {
    nbs6.using(
        (server, url) ->
            server
                .expect(requestTo(url + "/nbs/HomePage.do?method=patientSearchSubmit"))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess()));

    this.activePatient.maybeActive().map(this.requester::file).ifPresent(this.response::active);
  }
  ...
  public void i_am_redirected_to_the_nbs6_patient_file() throws Exception {
    String expected =
        activePatient
            ...
            .map(
                identifier ->
                    "/nbs/PatientSearchResults1.do?ContextAction=ViewFile&uid=%d"
                        .formatted(identifier.id()))
            .orElse("invalid");
