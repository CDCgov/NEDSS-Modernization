...
public class PatientFileMorbidityReportVerificationSteps {
  ...
  public void found() throws Exception {
    String local = this.activeReport.maybeActive().map(MorbidityReportIdentifier::local).orElse(null);
  ...
  public void within(final String jurisdiction) throws Exception {
    ...
    this.response.active().andExpect(
        jsonPath("$.[?(@.local=='%s' && @.jurisdiction=='%s')]",
            local, jurisdiction
        ).exists()
    );
  }
  ...
  public void reportedBy(final String first, final String last) throws Exception {
    String local = this.activeReport.maybeActive().map(MorbidityReportIdentifier::local).orElse(null);
    this.response.active().andExpect(
        jsonPath("$.[?(@.local=='%s' && @.reportingProvider.first=='%s' && @.reportingProvider.last=='%s')]",
            local, first, last
        ).exists()
    );
  }
  ...
  @Then("the patient file has the morbidity report containing a(n) {string} test with the result {string}")
  ...
  public void noTreatments() throws Exception {
    ...
    this.response.active()
        .andExpect(
            jsonPath("$.[?(@.local=='%s')].treatments]", local).isEmpty()
        );
  }
}
