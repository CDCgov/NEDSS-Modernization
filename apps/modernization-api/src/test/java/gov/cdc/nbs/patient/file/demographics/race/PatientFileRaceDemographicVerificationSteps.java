...
public class PatientFileRaceDemographicVerificationSteps {
  ...
  PatientFileRaceDemographicVerificationSteps(
  ...
  public void hasCategory(final String category) throws Exception {
    this.response.active()
        .andExpect(jsonPath("$.[?(@.race.value=='%s')]", category).exists());
  }
  ...
  public void doesNotHaveCategory(final String category) throws Exception {
    this.response.active()
        .andExpect(jsonPath("$.[?(@.race.value=='%s')]", category).isEmpty());
  }
  ...
  public void nthCategory(
      final int position,
      final String value
  ) throws Exception {
    this.response.active()
        .andExpect(jsonPath("$[%s].race.value", position - 1).value(value));
  }
  ...
  public void categoryIncludesDetail(final String detail, final String category) throws Exception {
    this.response.active()
        .andExpect(
            jsonPath(
                "$.[?(@.race.value=='%s')].detailed[?(@.value=='%s')]",
                category, detail
            ).exists()
        );
  }

  @Then("the patient file race demographics does not include {raceDetail} within the category {raceCategory}")
  ...
  public void categoryDoesNotIncludeDetails(final String category)
      throws Exception {
    this.response.active()
        .andExpect(
            jsonPath(
                "$.[?(@.race.value=='%s')].detailed[*]",
                category
            ).isEmpty()
        );
  }
