...
public class PatientFileDocumentsRequiringReviewSteps {
  ...
  public void no_documents_are_requiring_review(final String documentType) throws Exception {
    this.response.active().andExpect(jsonPath("$.[?(@.type=='%s')]", documentType).doesNotExist());
  }
  ...
  public void count(final String type, final int count) throws Exception {
    this.response.active().andExpect(jsonPath("$.[?(@.type=='%s')]", type).value(hasSize(count)));
  }

  @Then("the patient file has the {documentType} requiring review")
  public void the_correct_document_requiring_review_is_returned(final String documentType)
      throws Exception {
    String local =
        switch (documentType) {
    ...
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s')].[?(@.local=='%s')]", documentType, local).exists());
  }
  ...
  public void the_patient_document_requiring_review_is_electronic(final String documentType)
    ...
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s')].isElectronic", documentType)
  ...
  public void the_patient_document_requiring_review_was_sent_by(
      final String documentType, final String facility) throws Exception {
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s')].sendingFacility", documentType)
  ...
  public void the_patient_document_requiring_review_has_the_condition(
      final String documentType, final String title) throws Exception {
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s')].[?(@.condition=='%s')]", documentType, title).exists());
  }

  @Then("the {documentType} requiring review was received on {localDate} at {time}")
  public void receivedOn(final String documentType, final LocalDate on, final LocalTime at)
      throws Exception {
    this.response
        .active()
        .andExpect(
            jsonPath(
                    ...
                    documentType, LocalDateTime.of(on, at))
                .exists());
  }

  @Then("the {documentType} requiring review has the event date {localDate}")
  public void eventDate(final String documentType, final LocalDate on) throws Exception {
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s')].[?(@.eventDate=='%s')]", documentType, on).exists());
  }

  @Then("the {documentType} requiring review was reported by {string}")
  public void reportedBy(final String type, final String value) throws Exception {
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s' && @.reportingFacility=='%s')]", type, value).exists());
  }

  @Then("the {documentType} requiring review was ordered by the {string} facility")
  public void orderedBy(final String type, final String value) throws Exception {
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.type=='%s' && @.orderingFacility=='%s')]", type, value).exists());
  }

  @Then("the {documentType} requiring review was ordered by {string} {string} {string}")
  public void orderedBy(
      final String type, final String prefix, final String first, final String last)
      throws Exception {
    this.response
        .active()
        .andExpect(
  ...
  public void hasTreatment(final String type, final String value) throws Exception {
    this.response
        .active()
        .andExpect(jsonPath("$.[?(@.type=='%s')].treatments[?(@=='%s')]", type, value).exists());
  }
  ...
  public void noTreatments(final String type) throws Exception {
    this.response.active().andExpect(jsonPath("$.[?(@.type=='%s')].treatments]", type).isEmpty());
  }

  @Then("the {documentType} requiring review contains a(n) {string} test with the result {string}")
  public void hasNumericResultedTest(final String type, final String test, final String result)
      throws Exception {
    this.response
        .active()
        .andExpect(
  ...
  public void noResultedTests(final String type) throws Exception {
    this.response.active().andExpect(jsonPath("$.[?(@.type=='%s')].resultedTest]", type).isEmpty());
  }
