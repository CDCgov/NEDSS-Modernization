...
public class PatientProfileViewContactSteps {
  ...
  public void the_contact_is_viewed_from_the_patient_profile() {
    ...
    String request = "/nbs/api/profile/%d/contact/%d".formatted(patient, contact);

    activeResponse.active(
        authenticated
            .request(MockMvcRequestBuilders.get(request).queryParam("condition", "condition-value"))
            .andReturn()
            .getResponse());
  }
  ...
  public void i_am_redirected_to_classic_nbs_to_view_a_contact() {
    ...
    String expected =
        "/nbs/ContactTracing.do?method=viewContact&mode=View&Action=DSFilePath&contactRecordUid="
            + contact
            + "&DSInvestigationCondition=condition-value";
