...
public class InvestigationSearchResultVerificationSteps {
  ...
  public void there_is_only_one_investigation_search_result() throws Exception {
    this.response.active().andExpect(jsonPath("$.data.findInvestigationsByFilter.total").value(1));
  }
  ...
  public void there_are_no_investigation_search_results_available() throws Exception {
    this.response.active().andExpect(jsonPath("$.data.findInvestigationsByFilter.total").value(0));
  }
  ...
  private JsonPathResultMatchers matchingPath(final String field, final String position) {
    return switch (field.toLowerCase()) {
      case "birthday" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].personParticipations[*].birthTime",
              position);
      case "last name" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].personParticipations[*].lastName",
              position);
      case "first name" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].personParticipations[*].firstName",
              position);
      case "sex" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].personParticipations[*].currSexCd",
              position);
      case "patientid", "shortid" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].personParticipations[*].shortId",
              position);
      case "condition" ->
          jsonPath("$.data.findInvestigationsByFilter.content[%s].cdDescTxt", position);
      case "investigation id" ->
          jsonPath("$.data.findInvestigationsByFilter.content[%s].localId", position);
      case "investigator" ->
          jsonPath("$.data.findInvestigationsByFilter.content[%s].investigatorLastName", position);
      case "jurisdiction" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].jurisdictionCodeDescTxt", position);
      case "notification" ->
          jsonPath(
              "$.data.findInvestigationsByFilter.content[%s].notificationRecordStatusCd", position);
      case "start date" ->
          jsonPath("$.data.findInvestigationsByFilter.content[%s].startedOn", position);
      case "status" ->
          jsonPath("$.data.findInvestigationsByFilter.content[%s].investigationStatusCd", position);
      default ->
          throw new AssertionError(
              "Unexpected Investigation Search Result property %s".formatted(field));
    };
  }
}
