...
public class PatientFileGenderDemographicVerificationSteps {
  ...
  public void hasCurrent(final String value) throws Exception {
    this.response.active()
        .andExpect(jsonPath("$.current.value").value(value));
  }
  ...
  public void hasTransgender(final String value) throws Exception {
    this.response.active()
        .andExpect(jsonPath("$.transgenderInformation.value").value(value));
  }
  ...
  public void hasAdditional(final String value) throws Exception {
    this.response.active()
        .andExpect(jsonPath("$.additionalGender").value(value));
  }

  @Then("the patient file gender demographics has an unknown current gender with the reason being {sexUnknown}")
