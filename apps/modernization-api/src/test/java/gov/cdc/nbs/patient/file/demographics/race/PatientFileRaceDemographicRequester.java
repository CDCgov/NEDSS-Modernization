...
class PatientFileRaceDemographicRequester {
  ...
  PatientFileRaceDemographicRequester(final MockMvc mvc, final Authenticated authenticated) {
    this.mvc = mvc;
  ...
  ResultActions request(final PatientIdentifier patient) {
    try {
      ...
      throw new IllegalStateException(
          "An unexpected error occurred when viewing the patient race demographics.", exception);
    }
  }
}
