...
public class DeleteLegacyInvestigationSteps {
  ...
  public void the_viewed_legacy_investigation_has_no_associated_events() {
    activeContext.maybeActive().map(this::resolveRequestMatcher)
        .ifPresent(
            request -> server.expect(request)
  ...
  private RequestMatcher resolveRequestMatcher(final NBS6InvestigationRequest request) {
    return requestToUriTemplate("{base}/nbs/{location}?ContextAction={contextAction}&delete=true",
        classicUrl,
        request.location(),
