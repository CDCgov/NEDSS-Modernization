...
public class ContactRecordSteps {
  ...
  public void namesPrevious() {
    this.activeInvestigation
        ...
        .ifPresent(
            investigation ->
                mother.create(revisionMother.revise(availablePatients.previous()), investigation));
  }
  ...
  public void namedOn(final LocalDate value) {
    this.activeContactRecord.maybeActive().ifPresent(contact -> mother.namedOn(contact, value));
  }

  @Given("the contact record has a {priority} priority")
  public void priority(final String value) {
    this.activeContactRecord.maybeActive().ifPresent(contact -> mother.priority(contact, value));
  }

  @Given("the contact record has a {disposition} disposition")
  public void disposition(final String value) {
    this.activeContactRecord.maybeActive().ifPresent(contact -> mother.disposition(contact, value));
  }
