...
public class PatientSearchVerificationSteps {
  ...
  PatientSearchVerificationSteps(
      final Active<ResultActions> results, final Active<PatientIdentifier> patient) {
    ...
    this.results
        ...
        .andExpect(
            jsonPath("$.errors[*].message")
                .value(hasItem("User does not have permission to search for Inactive Patients")));
  }
  ...
  public void search_result_n_has_a_x_of_y(
    ...
    this.results.active().andExpect(pathMatcher.value(matchingValue(field, finalValue)));
  }

  @Then("the search results have a patient with the {nth} {string} equal to {string}")
  public void search_results_have_a_patient_with_a(
      final int nth, final String field, final String value) throws Exception {

    JsonPathResultMatchers pathMatcher = matchingPath(field, nth);

    this.results.active().andExpect(pathMatcher.value(matchingValue(field, value)));
  }

  @Then("the search results have a patient with a(n) {string} equal to {string}")
  public void search_results_have_a_patient_with_a(final String field, final String value)
      throws Exception {
  ...
  public void search_results_have_a_patient_without_a(final String field, final String value)
      throws Exception {

    JsonPathResultMatchers pathMatcher = matchingPath(field);

    this.results.active().andExpect(pathMatcher.value(not(matchingValue(field, value))));
  }
  ...
  public void search_results_have_a_patient_without_a(final String field) throws Exception {
    ...
    this.results.active().andExpect(pathMatcher.isEmpty());
  }
  ...
  private JsonPathResultMatchers matchingPath(
      final Integer position, final String field, final Integer order) {
    ...
    return switch (field.toLowerCase()) {
      ...
      case "birthday" ->
          jsonPath("$.data.findPatientsByFilter.content[%s].birthday", adjustedPostion);
      case "address" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].address",
              adjustedPostion, adjustedOrder);
      case "address type" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].type",
              adjustedPostion, adjustedOrder);
      case "address use" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].use",
              adjustedPostion, adjustedOrder);
      case "city" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].city",
              adjustedPostion, adjustedOrder);
      case "state" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].state",
              adjustedPostion, adjustedOrder);
      case "county" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].county",
              adjustedPostion, adjustedOrder);
      case "country" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].country",
              adjustedPostion, adjustedOrder);
      case "zip" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].addresses[%s].zipcode",
              adjustedPostion, adjustedOrder);
      case "gender", "sex" ->
          jsonPath("$.data.findPatientsByFilter.content[%s].gender", adjustedPostion);
      case "first name" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].names[%s].first",
              adjustedPostion, adjustedOrder);
      case "last name" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].names[%s].last",
              adjustedPostion, adjustedOrder);
      case "middle name" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].names[%s].middle",
              adjustedPostion, adjustedOrder);
      case "suffix" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].names[%s].suffix",
              adjustedPostion, adjustedOrder);
      ...
      case "phone", "phone number" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].phones[%s]", adjustedPostion, adjustedOrder);
      case "email", "email address" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].emails[%s]", adjustedPostion, adjustedOrder);
      case "identification type" ->
          jsonPath(
              "$.data.findPatientsByFilter.content[%s].identification[%s].type",
              adjustedPostion, adjustedOrder);
      ...
      case "patientid", "patient id", "short id" ->
          jsonPath("$.data.findPatientsByFilter.content[%s].shortId", adjustedPostion);
      case "local id" ->
          jsonPath("$.data.findPatientsByFilter.content[%s].localId", adjustedPostion);
  ...
  public void search_result_n_has_a_x_of_y(
      final String type, final String field, final String value) throws Exception {

    JsonPathResultMatchers pathMatcher =
        switch (field.toLowerCase()) {
          case "first" ->
              jsonPath("$.data.findPatientsByFilter.content[*].names[?(@.type=='%s')].first", type);
          case "last" ->
              jsonPath("$.data.findPatientsByFilter.content[*].names[?(@.type=='%s')].last", type);
  ...
  public void search_results_have_a_patient_with_the_phone(
      final String type, final String use, final String number) throws Exception {

    this.results
        .active()
        .andExpect(
            jsonPath(
                    "$.data.findPatientsByFilter.content[*].detailedPhones[?(@.type=='%s' && @.use=='%s')].number",
                    type, use)
                .value(number));
  }
  ...
  public void there_is_only_one_patient_search_result() throws Exception {
    this.results.active().andExpect(jsonPath("$.data.findPatientsByFilter.total").value(1));
  }
  ...
  public void there_is_are_x_patient_search_result(final int total) throws Exception {
    this.results.active().andExpect(jsonPath("$.data.findPatientsByFilter.total").value(total));
  }
