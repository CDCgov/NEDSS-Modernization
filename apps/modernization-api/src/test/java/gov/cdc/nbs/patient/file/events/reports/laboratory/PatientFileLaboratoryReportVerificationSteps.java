...
public class PatientFileLaboratoryReportVerificationSteps {
  ...
  public void within(final String area, final String jurisdiction) throws Exception {
    ...
    this.response
        ...
        .andExpect(
            jsonPath(
                    "$.[?(@.local=='%s' && @.programArea=='%s' && @.jurisdiction=='%s')]",
  ...
  public void orderedAt(final String value) throws Exception {
    ...
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.local=='%s' && @.orderingFacility=='%s')]", local, value).exists());
  }
  ...
  public void receivedOn(final LocalDate on, final LocalTime at) throws Exception {
    ...
    this.response
        ...
        .andExpect(
            jsonPath(
                    "$.[?(@.local=='%s' && @.receivedDate=='%s')]", local, LocalDateTime.of(on, at))
                .exists());
  }
  ...
  public void collectedOn(final LocalDate on) throws Exception {
    ...
    this.response
        .active()
        .andExpect(jsonPath("$.[?(@.local=='%s' && @.collectedDate=='%s')]", local, on).exists());
  }
  ...
  private void electronic(final boolean value) throws Exception {
    ...
    this.response
        .active()
        .andExpect(jsonPath("$.[?(@.local=='%s' && @.electronic==%s)]", local, value).exists());
  }

  @Then(
      "the patient file has the laboratory report containing a(n) {string} test with the result {string}")
  ...
  @Then(
      "the patient file has the laboratory report containing an ordered test with a specimen from the {string}")
  public void specimenSite(final String value) throws Exception {
    String local = this.activeReport.maybeActive().map(LabReportIdentifier::local).orElse(null);
    this.response
        .active()
        .andExpect(
            jsonPath("$.[?(@.local=='%s' && @.specimen.site=='%s')]", local, value).exists());
  }
