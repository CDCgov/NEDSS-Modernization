...
public class PatientPhoneDemographicSteps {
  ...
  PatientPhoneDemographicSteps(
      final PatientPhoneDemographicApplier applier,
  ...
  public void homePhone(
    ...
    this.patient.maybeActive()
        .ifPresent(found -> applier.withPhone(
                found,
                "PH", "H",
                countryCode,
                number,
                extension, RandomUtil.dateInPast()
            )
        );
  }
  ...
  public void phone(final String type, final String use, final String number) {
    this.patient.maybeActive()
        .ifPresent(found -> applier.withPhone(found, type, use, null, number, null, RandomUtil.dateInPast()));
  }
  ...
  public void phone(
    ...
    this.patient.maybeActive().ifPresent(
        identifier -> this.applier.withPhone(
            identifier,
            type,
            use,
            countryCode,
