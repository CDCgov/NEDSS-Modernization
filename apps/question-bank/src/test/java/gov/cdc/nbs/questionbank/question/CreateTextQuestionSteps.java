...
public class CreateTextQuestionSteps {
  ...
  public void the_text_question_is_create() throws Exception {
    Concept codeSystem = conceptFinder.find("CODE_SYSTEM", textRequest.getMessagingInfo().codeSystem()).orElseThrow();
    response.active()
        ...
        .andExpect(jsonPath("$.displayControl", equalTo(textRequest.getDisplayControl().intValue())))
        .andExpect(jsonPath("$.adminComments", equalTo(textRequest.getAdminComments())))
        .andExpect(
            jsonPath("$.dataMartInfo.defaultLabelInReport", equalTo(textRequest.getDataMartInfo().reportLabel())))
        ...
        .andExpect(jsonPath("$.dataMartInfo.rdbColumnName",
            equalTo(textRequest.getSubgroup() + "_" + textRequest.getDataMartInfo().rdbColumnName().toUpperCase())))
        .andExpect(
            jsonPath("$.dataMartInfo.dataMartColumnName", equalTo(textRequest.getDataMartInfo().dataMartColumnName())))
        .andExpect(
            jsonPath("$.messagingInfo.includedInMessage", equalTo(textRequest.getMessagingInfo().includedInMessage())))
        .andExpect(
            jsonPath("$.messagingInfo.messageVariableId", equalTo(textRequest.getMessagingInfo().messageVariableId())))
        .andExpect(jsonPath("$.messagingInfo.labelInMessage", equalTo(textRequest.getMessagingInfo().labelInMessage())))
        .andExpect(jsonPath("$.messagingInfo.codeSystem", equalTo(codeSystem.conceptName())))
        .andExpect(
            jsonPath("$.messagingInfo.requiredInMessage", equalTo(textRequest.getMessagingInfo().requiredInMessage())))
        .andExpect(jsonPath("$.messagingInfo.hl7DataType", equalTo(textRequest.getMessagingInfo().hl7DataType())));
  }
