...
import gov.cdc.nbs.questionbank.template.TemplateFinder;
...
class TemplateReaderTest {
  ...
  void searchTemplateExpanded() {
    Pageable pageable = Pageable.ofSize(25);
    TemplateSearchRequest search =
        new TemplateSearchRequest(1l, "templateNm", List.of("Draft"), "ConditionCd", "DataMartNm", "Active");
    when(templateRepository.searchTemplate(Mockito.anyLong(), Mockito.anyString(), Mockito.anyString(),
        Mockito.anyString(), Mockito.anyString(), Mockito.any(), Mockito.any()))
            .thenReturn(getTemplatePage(5, pageable));
