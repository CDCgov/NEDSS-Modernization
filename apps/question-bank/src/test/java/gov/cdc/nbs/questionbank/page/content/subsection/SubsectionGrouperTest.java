...
class SubsectionGrouperTest {
  ...
  void should_group_subsection() {
    ...
    GroupSubSectionRequest request = new GroupSubSectionRequest("BLOCK_X", getValidBatchList(), (int) 2);
  ...
  void should_not_update_null_blockName() {
    Long userId = 456L;
    GroupSubSectionRequest nullBlockNameRequest = new GroupSubSectionRequest(null, getValidBatchList(), 2);
    UpdateSubSectionException exception =
        assertThrows(UpdateSubSectionException.class, () -> grouper.group(1l, 100l, nullBlockNameRequest, userId));
    assertEquals("SubSection Block Name is required and only allows alphanumeric or underscore",
        exception.getMessage());

    GroupSubSectionRequest emptyBlockNameRequest = new GroupSubSectionRequest(" ", getValidBatchList(), 2);
    UpdateSubSectionException exception2 =
        assertThrows(UpdateSubSectionException.class, () -> grouper.group(1l, 100l, emptyBlockNameRequest, userId));
    assertEquals("SubSection Block Name is required and only allows alphanumeric or underscore",
  ...
  void testUpdateGroupQuestionBatch() {
    GroupSubSectionRequest.Batch batch = new GroupSubSectionRequest.Batch(123l, true, "TableHeader", 50);
    PageContentCommand.GroupSubsection command = new PageContentCommand.GroupSubsection(
        0,
        "BlockName",
        Arrays.asList(batch),
