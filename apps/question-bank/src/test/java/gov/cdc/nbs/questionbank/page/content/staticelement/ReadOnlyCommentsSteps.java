...
public class ReadOnlyCommentsSteps {
    ...
    public void i_update_a_read_only_comments_of(String key, String value) {
        switch (key) {
            case ("commentsText") -> this.updateRequest.active(UpdateStaticRequestHelper.withComments(updateRequest.active(), value));
            case ("adminComments") -> this.updateRequest.active(UpdateStaticRequestHelper.withReadOnlyCommentsAdminComments(updateRequest.active(), value));
        }
    }
