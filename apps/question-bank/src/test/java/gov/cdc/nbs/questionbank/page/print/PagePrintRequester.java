...
class PagePrintRequester {
  ...
  PagePrintRequester(final Authenticated authenticated, final MockMvc mvc) {
  ...
  ResultActions request(final long page) {
    try {
      return mvc.perform(this.authenticated.withSession(get("/api/v1/pages/{page}/print", page)));
