...
public class PagesResponseSteps {
  ...
  public void the_page_should_have_a_component_root() throws Exception {
    this.response.active()
        .andExpect(jsonPath("$.root").isNumber());
  }

  @Then("the page should have a(n) {string} equal to {string}")
  public void the_page_should_have_property_equal_to_value(final String property, final String value) throws Exception {
    JsonPathResultMatchers pathMatcher = resolvePageProperty(property);

    this.response.active()
        .andExpect(pathMatcher.value(containsStringIgnoringCase(value)));
  }
  ...
  public void the_page_should_have_a_child__named_with_a_property_equal_to_number(
    ...
    this.response.active()
        .andDo(print())
        .andExpect(pathMather.value(hasItem(equalTo(value))));
  }
  ...
  public void the_page_should_have_a_type_named_that_is(
    ...
    this.response.active()
        .andDo(print())
        .andExpect(pathMather.value(hasItem(equalTo(true))));
  }
  ...
  public void the_page_should_have_a_type_named_that_is_not(
    ...
    this.response.active()
        .andDo(print())
        .andExpect(pathMather.value(hasItem(not(equalTo(true)))));
  }

  private JsonPathResultMatchers resolveChildProperty(final String name, final String child, final String property) {

    String childPath = switch (child.toLowerCase()) {
