...
public class PageQuestionValidator {
  ...
  // Validate the provided datamart name is not already in use for the provided page, excluding the specified question
  public ValidationResponse validateDataMart(Long page, Long questionId, String datamart) {
    if (datamart == null) {
      return new ValidationResponse(false);
    }
    long count = factory.select(metadataTable.id.count())
        .from(metadataTable)
        .where(metadataTable.waTemplateUid.id.eq(page)
            .and(metadataTable.id.ne(questionId))
            .and(metadataTable.waRdbMetadatum.userDefinedColumnNm.eq(datamart.toUpperCase())))
        .fetchOne();
