...
public class QueryDSLOrderResolver {
  ...
  public static Stream<OrderSpecifier<?>> resolve(
    ...
    return pageable.getSort().stream()
        .flatMap(withOrdering(resolver))
        .filter(Objects::nonNull);
  }

  private static Function<Sort.Order, Stream<OrderSpecifier<?>>> withOrdering(final ExpressionResolver resolver) {
    return order -> resolver.resolve(order.getProperty())
  ...
  private static Function<Expression<?>, Stream<ComparableExpressionBase<?>>> onlyComparable() {
    return expression -> (expression instanceof ComparableExpressionBase<?> comparable) ?
  ...
  private static Function<ComparableExpressionBase<?>, OrderSpecifier<?>> withDirection(
    ...
    return comparable -> direction.isAscending()
        ? comparable.asc()
        : comparable.desc();
  }
