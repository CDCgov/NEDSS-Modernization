...
import gov.cdc.nbs.questionbank.page.information.change.UpdatePageInformationRequest;
...
public class PageUpdater {
   ...
   * @param request
   */
  public WaTemplate update(Long pageId, UpdatePageInformationRequest request) {
    ...
    if ("INV".equals(page.getBusObjType()) && !request.datamart().equals(page.getDatamartNm())) {
      boolean duplicateExists = pageRepository.existsByDatamartNmAndIdNot(request.datamart(), pageId);
      if (duplicateExists) {
  ...
  private PageCommand.UpdateInformation asUpdate(UpdatePageInformationRequest request) {
    return new PageCommand.UpdateInformation(
