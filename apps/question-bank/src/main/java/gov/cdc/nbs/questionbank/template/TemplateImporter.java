...
public class TemplateImporter {
  ...
  /**
   * Calls NBS Classic to perform the import of a template. The response from classic has a 302 Found status with the
   * `Location` header giving us the necessary information to determine if the import was a success. On a successful
   * import the header will be similar to:
   ...
   * 
   * On a failed import, the redirect will be null or contain "templateAlreadyExists" or "templateUid=TNF"
   * 
   * @param data
   * @return
   */
  public Template importTemplate(final MultipartFile file) {
    ...
    String uidString = location.substring(location.indexOf("templateUid=") + "templateUid=".length());
    try {
