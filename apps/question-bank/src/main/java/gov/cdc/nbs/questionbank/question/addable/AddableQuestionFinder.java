 ...
 */
@Component
public class AddableQuestionFinder {
  private final JPAQueryFactory factory;
  private final AddableQuestionTables tables;

  public AddableQuestionFinder(final JPAQueryFactory factory) {
    this.factory = factory;
    this.tables = new AddableQuestionTables();
  }

  public Page<AddableQuestion> find(final AddableQuestionCriteria criteria, final Long page, final Pageable pageable) {
    ...
    List<AddableQuestion> questions = findByIds(ids, orders);
  ...
  private JPAQuery<Tuple> findPossibleQuestionIds(final AddableQuestionCriteria criteria, final List<Long> inUse) {
  ...
  private List<AddableQuestion> findByIds(List<Long> ids, OrderSpecifier<?>[] orders) {
    return this.factory.select(
        // Basic question info
        this.tables.question().id,
        this.tables.question().questionType,
        ...
        .orderBy(orders)
        .stream()
        .map(AddableQuestionMapper::toQuestion)
        .toList();
  }

  private BooleanExpression applySearch(final AddableQuestionCriteria criteria, final List<Long> inUse) {
    return Stream.concat(applyCriteria(criteria), questionNotInUse(inUse))
        .reduce(BooleanExpression::and)
        .orElseThrow();
  }

  private Stream<BooleanExpression> applyCriteria(final AddableQuestionCriteria criteria) {
