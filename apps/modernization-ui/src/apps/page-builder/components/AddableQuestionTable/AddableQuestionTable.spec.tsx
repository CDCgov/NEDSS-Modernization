import { render } from '@testing-library/react';
import { AddableQuestionTable } from './AddableQuestionTable';
import { PageProvider } from 'page';
import { MemoryRouter } from 'react-router-dom';

describe('addable question table tests', () => {
    it('should render No Data when empty', () => {
        const { getByText } = render(
            <MemoryRouter>
                <PageProvider>
                    <AddableQuestionTable questions={[]} />
                </PageProvider>
            </MemoryRouter>
        );
        expect(getByText('No Data')).toBeInTheDocument();
    });

    it('should render headers when empty', () => {
        const { getAllByRole } = render(
            <MemoryRouter>
                <PageProvider>
                    <AddableQuestionTable questions={[]} />
                </PageProvider>
            </MemoryRouter>
        );

        const headers = getAllByRole('columnheader');
        expect(headers[1]).toHaveTextContent('Type');
        expect(headers[2]).toHaveTextContent('Unique ID');
        expect(headers[3]).toHaveTextContent('Unique name');
        expect(headers[4]).toHaveTextContent('Subgroup');
    });

    it('should display proper values for a question', async () => {
        const { findAllByRole } = render(
            <MemoryRouter>
                <PageProvider>
                    <AddableQuestionTable
                        questions={[
                            {
                                id: 1,
                                uniqueId: 'unique id',
                                uniqueName: 'unique name',
                                subgroupName: 'subgroup name',
                                type: 'PHIN'
                            }
                        ]}
                    />
                </PageProvider>
            </MemoryRouter>
        );

        const data = await findAllByRole('cell');
        expect(data[1]).toHaveTextContent('PHIN');
        expect(data[2]).toHaveTextContent('unique id');
        expect(data[3]).toHaveTextContent('unique name');
        expect(data[4]).toHaveTextContent('subgroup name');
    });
});
