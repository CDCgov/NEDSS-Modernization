...
const MatchingAttributeLabelMap: Map<MatchingAttribute, { label: string; isActive: (d: DataElements) => boolean }> =
    new Map([
        [
            MatchingAttribute.FIRST_NAME,
            { label: 'First name', isActive: (d: DataElements) => d.firstName?.active ?? false }
        ],
        [
            MatchingAttribute.LAST_NAME,
            { label: 'Last name', isActive: (d: DataElements) => d.lastName?.active ?? false }
        ],
        [MatchingAttribute.SUFFIX, { label: 'Suffix', isActive: (d: DataElements) => d.suffix?.active ?? false }],
        [
            MatchingAttribute.BIRTHDATE,
            { label: 'Date of birth', isActive: (d: DataElements) => d.dateOfBirth?.active ?? false }
        ],
        ...
        [
            ...
            { label: 'Social security number', isActive: (d: DataElements) => d.socialSecurity?.active ?? false }
        ],
        [
            MatchingAttribute.DRIVERS_LICENSE_NUMBER,
            { label: "Driver's license", isActive: (d: DataElements) => d.driversLicenseNumber?.active ?? false }
        ],
        [
            MatchingAttribute.MEDICAID_NUMBER,
            { label: 'Medicaid number', isActive: (d: DataElements) => d.medicaidNumber?.active ?? false }
        ],
        [
        ...
        [
            ...
            { label: 'Person number', isActive: (d: DataElements) => d.personNumber?.active ?? false }
        ],
        [
            MatchingAttribute.VISA_PASSPORT,
            { label: 'VISA / Passport number', isActive: (d: DataElements) => d.visaPassport?.active ?? false }
        ],
        [
            MatchingAttribute.WIC_IDENTIFIER,
            { label: 'WIC identifier', isActive: (d: DataElements) => d.wicIdentifier?.active ?? false }
        ]
    ]);
