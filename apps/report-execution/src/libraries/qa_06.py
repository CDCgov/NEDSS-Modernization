    ...
    * Sorting differs slightly from QA06.sas.  In original SAS file, there is only one
    ...
    query = f"""
        ...
        INV AS (
          ...
          FROM [{nbs_rdb}].[dbo].[INVESTIGATION]
            JOIN [{nbs_rdb}].[dbo].[EVENT_METRIC]
                ON INVESTIGATION.CASE_UID = EVENT_METRIC.EVENT_UID
            JOIN [{nbs_rdb}].[dbo].[USER_PROFILE]
        ...
        PATIENT_CASES AS (
          ...
          FROM STD_HIV_DATAMART SHD
            ...
            LEFT OUTER JOIN {nbs_rdb}.[dbo].[MORBIDITY_REPORT_EVENT] MRE
              ON SHD.INVESTIGATION_KEY = MRE.INVESTIGATION_KEY
            LEFT OUTER JOIN {nbs_rdb}.[dbo].[MORBIDITY_REPORT] MR
