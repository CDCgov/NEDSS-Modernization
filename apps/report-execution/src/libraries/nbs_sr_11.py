...
def execute(
    ...
    * Capitalized the `Y` in `Year`
    ...
    content = trx.query(
        ...
        FROM subset
        GROUP BY state, state_cd, county, phc_code_short_desc, datepart(year, event_date)
        ORDER BY state, state_cd, county, phc_code_short_desc, datepart(year, event_date)
        """
    )
