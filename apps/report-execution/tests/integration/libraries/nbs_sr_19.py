...
class TestIntegrationNbsSr19Library:
    ...
    def test_execute_report_invalid_params_raises_error(self):
        ...
        with pytest.raises(ValueError, match="library_params must be a dictionary containing 'count_column'"):
    ...
    def test_execute_report_handles_null_and_diverse_statuses(self):
        """Verifies that null statuses and alternate text fall into non_counted_cases."""
        status_query = """
            SELECT CAST('2025-06-01' AS DATE) AS COUNT_DATE, 'Count as a TB Case' AS count_status
            UNION ALL
            SELECT CAST('2025-06-02' AS DATE) AS COUNT_DATE, NULL AS count_status
            UNION ALL
            SELECT CAST('2025-06-03' AS DATE) AS COUNT_DATE, 'Suspected Case' AS count_status
        """
        report_spec = ReportSpec.model_validate({
            'is_export': True,
            'is_builtin': True,
