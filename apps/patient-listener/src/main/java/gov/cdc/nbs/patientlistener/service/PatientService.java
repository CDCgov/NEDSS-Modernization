...
public class PatientService {
    ...
    public void handlePatientCreate(String message, String key) {
        ...
        try {
            log.debug("Attempting to validate permissions for user: {}", username);
            ...
            log.warn("Failed to find user credentials for userId: {}", username);
            sendPatientCreateStatus(false, key, "Failed to find user in system: " + username);
            return;
        }
    ...
    private Person createPatient(PatientCreateRequest request, Long userId) {
        ...
        person.setId(request.getPatientId());
        person.setPersonParentUid(person);
        person.setLocalId(request.getPatientLocalId());
        person.setNbsEntity(new NBSEntity(request.getPatientId(), "PSN"));
