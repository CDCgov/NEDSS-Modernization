...
errorElts[i++]=getElementByIdOrByName("NBS146"); 
 ...
 return {elements : errorElts, labels : errorMsgs}
  ...
  function ruleRequireIfNBS2097273()
{
 var foo = [];
...
pgDisableElement('INV503');
  ...
  function ruleHideUnhNBS2097280()
{
 var foo = [];
$j('#NBS209 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
...
pgHideElement('NBS211_2');
  ...
  function ruleHideUnhNOT1207281()
{
 var foo = [];
...
pgDisableElement('102957003');
  ...
  function ruleEnDis75204_87283()
{
 var foo = [];
$j('#75204_8 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('75200_6');
pgEnableElement('75163_6');
pgEnableElement('NBS390');
 } else { 
pgDisableElement('75200_6');
pgDisableElement('75163_6');
pgDisableElement('NBS390');
 }   
}
      
  function ruleEnDis75197_47284()
{
 var foo = [];
$j('#75197_4 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('3',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
...
pgDisableElement('NBS389');
  ...
  function ruleEnDisNBS1737285()
{
 var foo = [];
$j('#NBS173 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('G',foo) > -1) || ($j.inArray('G - Insufficient Info to Begin Investigation'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('H',foo) > -1) || ($j.inArray(' H - Unable to Locate'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('J',foo) > -1) || ($j.inArray(' J - Located'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('L',foo) > -1) || ($j.inArray(' Not Examined and/or Interviewed'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('NBS392');
 } else { 
pgDisableElement('NBS392');
 }   
}
      
  function ruleEnDisINV2907286()
{
 var foo = [];
$j('#INV290 :selected').each(function(i, selected){
 ...
 if(($j.inArray('8',foo) > -1) || ($j.inArray('Reagin Ab [Presence] in Cerebral spinal fluid by VDRL'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('2',foo) > -1) || ($j.inArray(' Reagin Ab [Presence] in Serum by RPR'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('9',foo) > -1) || ($j.inArray(' Reagin Ab [Presence] in Serum by VDRL'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('6',foo) > -1) || ($j.inArray(' Unknown Non-Treponemal Test Type'.replace(/^\s+|\s+$/g,''),foo) > -1)){
...
pgDisableElement('STD123_1');
  ...
  function ruleEnDisINV290_MTH7287()
{
 var foo = [];
$j('#INV290_MTH :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('8',foo) > -1) || ($j.inArray('Reagin Ab [Presence] in Cerebral spinal fluid by VDRL'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('2',foo) > -1) || ($j.inArray(' Reagin Ab [Presence] in Serum by RPR'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('9',foo) > -1) || ($j.inArray(' Reagin Ab [Presence] in Serum by VDRL'.replace(/^\s+|\s+$/g,''),foo) > -1) || ($j.inArray('6',foo) > -1) || ($j.inArray(' Unknown Non-Treponemal Test Type'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('STD123_MTH');
 } else { 
pgDisableElement('STD123_MTH');
 }   
}
      
  function ruleEnDisNBS1687288()
{
 var foo = [];
$j('#NBS168 :selected').each(function(i, selected){
 ...
 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('NBS169');
 } else { 
pgDisableElement('NBS169');
 }   
}
      
  function ruleEnDisNBS1117289()
{
 var foo = [];
$j('#NBS111 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
...
if(foo.length>0 && foo[0] != '') {
pgSubSectionEnabled('NBS_INV_STD_UI_9');
 } else { 
pgSubSectionDisabled('NBS_INV_STD_UI_9');
     ...
     function pgCheckForDynamicRuleErrorsOnSubmit() {
        ...
        retVal = ruleDCompINV1377267();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompDEM1157268();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompDEM1157269();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompNBS1747271();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompNBS1477272();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompINV1477275();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
      ...
      function pgCheckDynamicRulesOnLoad() {
           	...
           	stdCheckDispositionValue();stdReferalBasisExposureFreqEnabReq();stdResetPageStateOnLoad();stdSetupDiagnosisOptions();ruleEnDisINV1287262();ruleEnDisINV1457263();ruleEnDisINV1507264();ruleEnDisINV1527265();ruleEnDisDEM1277270();ruleEnDisNBS1117274();ruleEnDisINV1787276();ruleEnDisDEM1137277();ruleEnDisDEM1557278();ruleEnDisINV5027279();ruleHideUnhNBS2097280();ruleHideUnhNOT1207281();ruleEnDisSTD1027282();ruleEnDis75204_87283();ruleEnDis75197_47284();ruleEnDisNBS1737285();ruleEnDisINV2907286();ruleEnDisINV290_MTH7287();ruleEnDisNBS1687288();ruleEnDisNBS1117289();ruleRequireIfNBS2097273();
	...
	<!-- === batch subsection add edit check functions (if any) follow ===-->
        ...
        function pgNBS_INV_STD_UI_81BatchAddFunction()
     	    { 
    			var errorLabels = new Array();
    			var retVal;
    			var retRule;
        	...
        	retVal=pgCheckForErrorsOnBatchSubsection('NBS_INV_STD_UI_81');
         	if (retVal != null && retVal != undefined) { errorLabels = errorLabels.concat(retVal); } 

        
        	if (errorLabels.length > 0) {
        	displayErrors('NBS_INV_STD_UI_81errorMessages', errorLabels); return false;}
		$j('#NBS_INV_STD_UI_81errorMessages').css("display", "none");
		return true;
		}
        function pgNBS_INV_STD_UI_86BatchAddFunction()
     	    { 
    			var errorLabels = new Array();
    			var retVal;
    			var retRule;
        	...
        	retVal=pgCheckForErrorsOnBatchSubsection('NBS_INV_STD_UI_86');
         	if (retVal != null && retVal != undefined) { errorLabels = errorLabels.concat(retVal); } 

        
        	if (errorLabels.length > 0) {
        	displayErrors('NBS_INV_STD_UI_86errorMessages', errorLabels); return false;}
		$j('#NBS_INV_STD_UI_86errorMessages').css("display", "none");
		return true;
		}
     ...
     <!-- ################### PAGE TAB ###################### - - -->
      <layout:tabs width="100%" styleClass="tabsContainer">
      		...
      		<jsp:include page="CaseManagement.jsp"/>
      		 ...
      		 <layout:tab key="Congenital Syphilis">     
      		<jsp:include page="CongenitalSyphilis.jsp"/>
             	</layout:tab>               	
