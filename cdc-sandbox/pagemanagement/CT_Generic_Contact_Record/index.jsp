...
pgDisableElement('DEM128');
  ...
  function ruleEnDisCON1287356()
{
 var foo = [];
$j('#CON128 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('CON129');
 } else { 
pgDisableElement('CON129');
 }   
}
      
  function ruleEnDisCON1177357()
{
 var foo = [];
$j('#CON117 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('CON118');
 } else { 
pgDisableElement('CON118');
 }   
}
      
  function ruleEnDisCON1207358()
{
 var foo = [];
$j('#CON120 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('N',foo) > -1) || ($j.inArray('No'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('CON122');
 } else { 
pgDisableElement('CON122');
 }   
}
      
  function ruleEnDisCON1207359()
{
 var foo = [];
$j('#CON120 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 ...
 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('CON124');
pgEnableElement('CON121');
pgEnableElement('CON123');
 } else { 
pgDisableElement('CON124');
pgDisableElement('CON121');
pgDisableElement('CON123');
 }   
}
      
  function ruleEnDisCON1237360()
{
 var foo = [];
$j('#CON123 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('N',foo) > -1) || ($j.inArray('No'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('CON125');
 } else { 
pgDisableElement('CON125');
 }   
}
      
  function ruleEnDisCON1147361()
{
 var foo = [];
$j('#CON114 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });
...
pgDisableElement('NBS273');
     ...
     function pgCheckForDynamicRuleErrorsOnSubmit() {
      ...
      function pgCheckDynamicRulesOnLoad() {
           	...
           	ruleEnDisDEM1277355();ruleEnDisCON1287356();ruleEnDisCON1177357();ruleEnDisCON1207358();ruleEnDisCON1207359();ruleEnDisCON1237360();ruleEnDisCON1147361();ruleEnDisDEM1557362();
    ...
    pgCheckOnloadOtherEntryAllowedFields();pgCheckDynamicRulesOnLoad();pgCheckForFieldsToHighlightOnEdit();pgPopulateCounties();addRolePresentationToTabsAndSections();">
     ...
     <!-- ################### PAGE TAB ###################### - - -->
      <layout:tabs width="100%" styleClass="tabsContainer">
      		...
      		<jsp:include page="ContactFollowUp.jsp"/>
             	</layout:tab>               	
