...
pgDisableElement('INV503');
  ...
  function ruleHideUnhNOT1208781()
{
 var foo = [];
...
pgHideElement('NOT120SPEC_2');
  ...
  function ruleHideUnhINV1788782()
{
 var foo = [];
...
pgHideElement('INV579_2');
  ...
  function ruleEnDisINV12708783()
{
 var foo = [];
$j('#INV1270 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('INV831');
 } else { 
pgDisableElement('INV831');
 }   
}
      
  function ruleEnDisTRAVEL388784()
{
 var foo = [];
...
pgEnableElement('90366_6');
 } else { 
pgDisableElement('TRAVEL05');
pgDisableElement('90366_6');
 }   
}
      
  function ruleEnDis90366_68785()
{
 var foo = [];
$j('#90366_6 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('91514_0');
 } else { 
pgDisableElement('91514_0');
 }   
}
      
  function ruleEnDisINV7408786()
{
 var foo = [];
$j('#INV740 :selected').each(function(i, selected){
 ...
 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgSubSectionEnabled('NBS_UI_37');
pgSubSectionEnabled('NBS_UI_42');
 } else { 
pgSubSectionDisabled('NBS_UI_37');
pgSubSectionDisabled('NBS_UI_42');
 }   
}
      
  function ruleEnDisINV6368787()
{
 var foo = [];
$j('#INV636 :selected').each(function(i, selected){
 foo[i] = $j(selected).val();
 });

 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('INV1120');
 } else { 
pgDisableElement('INV1120');
 }   
}
      
  function ruleEnDisINV1698788()
{
 var foo = [];
$j('#INV169 :selected').each(function(i, selected){
 ...
 if(($j.inArray('50263',foo) > -1) || ($j.inArray('Candida auris'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgEnableElement('INV1109');
 } else { 
pgDisableElement('INV1109');
 }   
}
     ...
     function pgCheckForDynamicRuleErrorsOnSubmit() {
        ...
        retVal = ruleDCompINV1378777();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompDEM1158778();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompDEM1158779();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
      ...
      function pgCheckDynamicRulesOnLoad() {
           	...
           	ruleEnDisINV1288770();ruleEnDisINV1458771();ruleEnDisDEM1278772();ruleEnDisINV1508773();ruleEnDisINV1528774();ruleEnDisDEM1138775();ruleEnDisINV5028780();ruleHideUnhNOT1208781();ruleHideUnhINV1788782();ruleEnDisINV12708783();ruleEnDisTRAVEL388784();ruleEnDis90366_68785();ruleEnDisINV7408786();ruleEnDisINV6368787();ruleEnDisINV1698788();
	...
	<!-- === batch subsection add edit check functions (if any) follow ===-->
        ...
        function pgNBS_UI_37BatchAddFunction()
     	    { 
    			var errorLabels = new Array();
    			var retVal;
    			var retRule;
        	...
        	retVal=pgCheckForErrorsOnBatchSubsection('NBS_UI_37');
         	if (retVal != null && retVal != undefined) { errorLabels = errorLabels.concat(retVal); } 

        
        	if (errorLabels.length > 0) {
        	displayErrors('NBS_UI_37errorMessages', errorLabels); return false;}
		$j('#NBS_UI_37errorMessages').css("display", "none");
		return true;
		}
     ...
     <!-- ################### PAGE TAB ###################### - - -->
      <layout:tabs width="100%" styleClass="tabsContainer">
      		...
      		<jsp:include page="CaseInfo.jsp"/>
      		 ...
      		 <layout:tab key="Candida auris">     
      		<jsp:include page="Candidaauris.jsp"/>
             	</layout:tab>               	
