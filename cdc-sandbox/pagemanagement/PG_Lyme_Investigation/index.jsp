...
pgDisableElement('INV503');
  ...
  function ruleHideUnhNOT1208655()
{
 var foo = [];
...
pgHideElement('NOT120SPEC_2');
  ...
  function ruleHideUnhINV1788656()
{
 var foo = [];
...
pgDisableElement('NBS338_OTH');
  ...
  function ruleEnDis2556330018658()
{
 var foo = [];
$j('#255633001 :selected').each(function(i, selected){
 ...
 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgSubSectionEnabled('NBS_UI_45');
 } else { 
pgSubSectionDisabled('NBS_UI_45');
 }   
}
      
  function ruleEnDisINV7408659()
{
 var foo = [];
$j('#INV740 :selected').each(function(i, selected){
 ...
 if(($j.inArray('Y',foo) > -1) || ($j.inArray('Yes'.replace(/^\s+|\s+$/g,''),foo) > -1)){
pgSubSectionEnabled('NBS_UI_48');
 } else { 
pgSubSectionDisabled('NBS_UI_48');
 }   
}
     ...
     function pgCheckForDynamicRuleErrorsOnSubmit() {
        ...
        retVal = ruleDCompINV1378651();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompDEM1158652();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
        retVal = ruleDCompDEM1158653();
     	   if (retVal != null && retVal.elements != undefined) { errorElts = errorElts.concat(retVal.elements); errorLabels = errorLabels.concat(retVal.labels); }  
      ...
      function pgCheckDynamicRulesOnLoad() {
           	...
           	ruleEnDisINV1288644();ruleEnDisINV1458645();ruleEnDisDEM1278646();ruleEnDisINV1508647();ruleEnDisINV1528648();ruleEnDisDEM1138649();ruleEnDisINV5028654();ruleHideUnhNOT1208655();ruleHideUnhINV1788656();ruleEnDisNBS3388657();ruleEnDis2556330018658();ruleEnDisINV7408659();
	...
	<!-- === batch subsection add edit check functions (if any) follow ===-->
        ...
        function pgNBS_UI_48BatchAddFunction()
     	    { 
    			var errorLabels = new Array();
    			var retVal;
    			var retRule;
        	...
        	retVal=pgCheckForErrorsOnBatchSubsection('NBS_UI_48');
         	if (retVal != null && retVal != undefined) { errorLabels = errorLabels.concat(retVal); } 

        
        	if (errorLabels.length > 0) {
        	displayErrors('NBS_UI_48errorMessages', errorLabels); return false;}
		$j('#NBS_UI_48errorMessages').css("display", "none");
		return true;
		}
