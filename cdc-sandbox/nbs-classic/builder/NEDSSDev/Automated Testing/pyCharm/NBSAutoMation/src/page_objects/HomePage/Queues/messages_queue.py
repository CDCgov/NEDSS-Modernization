...
class MessageQueue(BasePageObject):
    ...
    MESSAGE_QUEUE_XPATH = '//*[@id="myQueues"]/div/div[3]/ul[1]/li[7]/a'

    # Sort/filter Results Grid
    column1_name = "New?"
    column2_name = "Date"
    column3_name = "Submitted By"
    column4_name = "Message"
    column5_name = "Patient"
    column6_name = "Condition"

    # # Sort/filter Results Grid by Id
    column1_name_id = "messageStatus"
    column2_name_id = "sdate"
    column3_name_id = "submittedBy"
    column4_name_id = "messageText"
    column5_name_id = "patient"
    column6_name_id = "testCond"
    ...
    column5_header_index = 4
    column6_header_index = 3
    ...
    def validateSortingOnResultsGrid(self):
        ...
        print("grid_columns!!!!!!!!!!!!!!!!!!", grid_columns1, grid_columns2)
        time.sleep(2)
        self.sortResultsGridColumnsOneByOne(grid_columns2, "Asc")
        self.sortResultsGridColumnsOneByOne(grid_columns1, "Desc")
    ...
    def validateFilterOnResultsGrid(self):
        ...
        # canhe this metod
        self.msgQueueFilterResultsGridDataByColumn(filter_columns)

        return True

    def msgQueueFilterResultsGridDataByColumn(self, filter_columns):
        for key, value in filter_columns.items():
            if str(value[0]) == "checkbox":
                ...
                time.sleep(1)
                self.MsgQueuePerformColumnCheckBoxPopupSearch("OPENINV", str(key), value)
            elif str(value[0]) == "textfield":
                self.performButtonClick(NBSConstants.TYPE_CODE_XPATH, str(value[1]))
                self.msgQueuePerformColumnTextFieldPopupSearch("OPENINV", str(key), value)
            time.sleep(1)
    ...
    def MsgQueuePerformColumnCheckBoxPopupSearch(self, component_name, key, value_list):
        if component_name == "OPENINV":
            if self.msgQueuePerformCheckboxFilterOnTheResultsGrid(value_list[2]):
                self.logger.info(str(key) + " Results Grid Filter Test Passed")

    def msgQueuePerformColumnTextFieldPopupSearch(self, component_name, key, value_list):
        if component_name == "OPENINV":
            if self.msgQueuePerformTextFieldFilterOnTheResultsGrid(key, value_list[4]):
                self.logger.info(str(key) + " Results Grid Filter Test Passed")

    def msgQueuePerformCheckboxFilterOnTheResultsGrid(self, replace):
        ...
        if len(option_checkboxes) > 5:
            ...
            if select_checkbox_list is not None:
                if len(select_checkbox_list) > 1:
                    ...
                    print("second check",
                          checkbox_options_popup + "[" + str(int(select_checkbox_list[1]) + 1) + "]td/input")
    ...
    def msgQueuePerformTextFieldFilterOnTheResultsGrid(self, key, column_id):
    ...
    def getSearchText(self, key):
        search_text = None
        if key == "Patient":
            search_text = (self.findElement(NBSConstants.TYPE_CODE_XPATH, '//*[@id="parent"]/tbody/tr[1]/td[7]')
                           .get_attribute('text'))
    ...
    def validateConditionLInkInTheResultsPage(self):
        try:
            ...
            self.performButtonClick_web(NBSConstants.TYPE_CODE_LINK_TEXT,
                                        "Return to Messages Queue")
        except:
            return False
        return True

    def validateMarkAsReadLInkInTheResultsPage(self):
        self.driver.find_element(By.XPATH, '//*[@id="parent"]/tbody/tr[1]/td[1]/img')
        return True

    @staticmethod
    def getRandomlySelectCheckBoxOptions(check_boxes_size):
        MessageQueue.logger.info(check_boxes_size)
        list_random_cb = None
        if check_boxes_size is not None and int(check_boxes_size) > 1:
            ...
            list_random_cb = [1]
        MessageQueue.logger.info(list_random_cb)
        return list_random_cb

    def validateMessageQueueDelete(self):
