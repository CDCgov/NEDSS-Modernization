...
class DocumentReqReview(BasePageObject):
    ...
    report = {
        "Case": '// *[ @ id = "parent"] / thead / tr / th[2] / div / label[3] / input',
              "lab": '// *[ @ id = "parent"] / thead / tr / th[2] / div / label[4] / input',
              "morbidity": '// *[ @ id = "parent"] / thead / tr / th[2] / div / label[5] / input'}
    # locators
    ...
    DOCUMENT_REQ_REVIEW_XPATH= "//*[@id='myQueues']/div/div[3]/ul[1]/li[6]/a"
    ...
    def performCheckboxFilterOnTheResultsGrid(self, replace_index):
        ...
        if len(option_checkboxes) > 5:
            ...
            option_checkboxes_correct_size = len(self.findElements(NBSConstants.TYPE_CODE_XPATH, checkbox_options_popup
                                                                   + "[@class='pText']"))
            select_checkbox_list = self.getRandomlySelectCheckBoxOptions(option_checkboxes_correct_size)  # multiple checkboxes
    ...
    def getRandomlySelectCheckBoxOptions(check_boxes_size):
        DocumentReqReview.logger.info(check_boxes_size)
        list_random_cb = None
        if check_boxes_size is not None and int(check_boxes_size) > 1:
            ...
            list_random_cb = [1]
        DocumentReqReview.logger.info(list_random_cb)
    ...
    def validateMarkAsReviewdSubFormOnTheResultsPage(self):
        try:
            ...
            # print("std_list_records", std_list_records)
            # print("non_std_list_records", non_std_list_records)
    ...
    def validateMarkAsReviewdBulkOnTheResultsPage(self):
        # click img filter
        self.driver.find_element(By.XPATH, self.column1_xpath).click()
        loop = self.driver.find_elements(By.XPATH,
                                         "//*[@id='parent']/thead/tr/th[2]/div/label/following-sibling::*[@class='pText checked']")
        self.driver.find_element(By.XPATH, "//*[@id='parent']/thead/tr/th[2]/div/label[1]/input[2]").click()
        # print("loop", len(loop))
        k = 2
        for i in range(len(loop)):
            ...
            time.sleep(1)
            self.driver.find_element(By.XPATH, '//*[@id="removeFilters"]/table/tbody/tr/td[2]/a/font').click()
            self.driver.find_element(By.XPATH, self.column1_xpath).click()
            self.driver.find_element(By.XPATH, '// *[ @ id = "parent"] / thead / tr / th[2] / div / label[2] / input').click()
            self.driver.find_element(By.XPATH,'// *[ @ id = "parent"] / thead / tr / th[2] / div / label[{0}] / input'.format(j)).click()
            # clikc ok
            self.driver.find_element(By.XPATH, "//*[@id='parent']/thead/tr/th[2]/div/label[1]/input[1]").click()

            self.driver.find_element(By.XPATH, '//*[@id="parent"]/thead/tr/th[1]/input').click()
            first_page_records = self.driver.find_elements(By.XPATH,'//*[@id="parent"]/tbody/tr')
            element = self.driver.find_element(By.XPATH, '//*[@id="parent"]/tbody/tr').text
            std_list_records = []
            non_std_list_records = []
            if element != "Nothing found to display.":
                for index in range(1, int(len(first_page_records)) + 1):
                    std_program_area = self.findElement(NBSConstants.TYPE_CODE_XPATH,
                                                        "//*[@id='parent']/tbody/tr[" + str(index) + "]/td[10]")
                ...
                # print('std_list_records',std_list_records)
                # print('non_std_list_records',non_std_list_records)
                self.driver.find_element(By.LINK_TEXT, 'Mark As Reviewed').click()

                if non_std_list_records == []:
    ...
    def select_checkbox(self, selectcheck):
        for i in selectcheck:
            self.performButtonClick(NBSConstants.TYPE_CODE_XPATH,
                                    '//*[@id="parent"]/thead/tr/th[6]/div/label[{}]/input'.format(i))
    def validateMarkAsReviewdForNONSTDSubFormOnTheResultsPage(self):
          self.driver.find_element(By.XPATH, self.column1_xpath).click()
          loop = self.driver.find_elements(By.XPATH,"//*[@id='parent']/thead/tr/th[2]/div/label/following-sibling::*[@class='pText checked']")
          self.driver.find_element(By.XPATH, "//*[@id='parent']/thead/tr/th[2]/div/label[1]/input[2]").click()
          # print("loop",len(loop))
          k = 2
          for i in range(len(loop)):
            ...
            self.driver.find_element(By.XPATH,
                                     '// *[ @ id = "parent"] / thead / tr / th[2] / div / label[2] / input').click()
            self.driver.find_element(By.XPATH, '// *[ @ id = "parent"] / thead / tr / th[2] / div / label[{0}] / input'.format(j) ).click()
            # clikc ok
            self.driver.find_element(By.XPATH, "//*[@id='parent']/thead/tr/th[2]/div/label[1]/input[1]").click()
            first_page_records = self.driver.find_elements(By.XPATH, '//*[@id="parent"]/tbody/tr')
            ...
            if element != "Nothing found to display.":
                ...
                if non_std_list_records != []:
                    self.performMarkAsReviewdOnSelectedRecord(non_std_list_records, std_list="false")
                    self.logger.info("Non STD Records are Mark As Reviewd")
                    self.setWaitTimeAfterEachAction(NBSConstants.ENABLE_WAITING_TIME, NBSConstants.WAITING_TIME_IN_SECONDS)

          return True

    def validateMarkAsReviewdForSTDSubFormOnTheResultsPage(self):
          self.driver.find_element(By.XPATH, self.column1_xpath).click()
          loop = self.driver.find_elements(By.XPATH,"//*[@id='parent']/thead/tr/th[2]/div/label/following-sibling::*[@class='pText checked']")
          self.driver.find_element(By.XPATH, "//*[@id='parent']/thead/tr/th[2]/div/label[1]/input[2]").click()
          # print("loop",len(loop))
          ...
          for i in range(len(loop)):
                ...
                if element != "Nothing found to display.":
                    ...
                    if std_list_records != []:
                        self.performMarkAsReviewdOnSelectedRecord(std_list_records, std_list="true")
                        self.logger.info("STD Records are Mark As Reviewd")
                        self.setWaitTimeAfterEachAction(NBSConstants.ENABLE_WAITING_TIME, NBSConstants.WAITING_TIME_IN_SECONDS)
