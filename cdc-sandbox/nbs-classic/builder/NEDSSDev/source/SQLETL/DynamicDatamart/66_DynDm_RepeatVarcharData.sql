...
--  exec  rdb.dbo.DynDm_RepeatVarcharData_sp  231207132840 ,'RABIES' ;

--DECLARE  @batch_id BIGINT = 999;  DECLARE @DATAMART_NAME  VARCHAR(100) = 'GENERIC_V2';
...
GO
			 ...
			 IF OBJECT_ID('rdb.dbo.DynDm_RepeatVarcharData_sp', 'P') IS NOT NULL
				BEGIN
				DROP PROCEDURE dbo.DynDm_RepeatVarcharData_sp;
				END;
...
CREATE PROCEDURE [dbo].DynDm_RepeatVarcharData_sp
...
--DECLARE  @batch_id BIGINT = 999;  DECLARE @DATAMART_NAME  VARCHAR(100) = 'GENERIC_V2';
    ...
    INSERT INTO rdb.[dbo].[job_flow_log] (
          ...
          ,[row_count]
		   ...
		   @batch_id
           ,'DYNAMIC_DATAMART'
           ,'RDB.DBO.DynDm_RepeatVarcharData_sp '+ @DATAMART_NAME
		   ,'START'
		   ,@Proc_Step_no
		   ,@Proc_Step_Name
           ,0
...
--%MACRO 66_DyDm_REPEATVARCHARDATA();
...
--DECLARE  @batch_id BIGINT = 999;  DECLARE  @DATAMART_NAME VARCHAR(100) = 'GENERIC_V2'; 
 ...
 SELECT  distinct tmp_INIT.DATAMART_NM, RDB_COLUMN_NM, USER_DEFINED_COLUMN_NM, NBS_RDB_METADATA.BLOCK_PIVOT_NBR, BLOCK_NM 
 ...
 FROM NBS_ODSE..NBS_UI_METADATA 
    ...
    INNER JOIN rdb.dbo.tmp_INIT ON NBS_UI_METADATA.INVESTIGATION_FORM_CD = tmp_INIT.FORM_CD
...
LENGTH RDB_COLUMN_NM2 $30;
	...
	RDB_COLUMN_NM2= LTRIM(  RTRIM(RDB_COLUMN_NM3) || '_OTH';
	USER_DEFINED_COLUMN_NM = LTRIM(  RTRIM(USER_DEFINED_COLUMN_NM)|| '_OTH';
	RDB_COLUMN_NM= RDB_COLUMN_NM2;
DROP RDB_COLUMN_NM2 RDB_COLUMN_NM3;
RUN;
*/

--DX select SUBSTRING('IPO_DD_R_PDI_CODED_MULTI_OTH',1,22)
...
-- CREATE TABLE METADATA_UNIT AS 
 ...
 VALUES ( @batch_id ,'DYNAMIC_DATAMART' ,'RDB.DBO.DynDm_RepeatVarcharData_sp '+@DATAMART_NAME  ,'START' ,@Proc_Step_no ,@Proc_Step_Name ,@ROWCOUNT_NO );
 ...
 SELECT  distinct tmp_INIT.DATAMART_NM,
 --RDB_COLUMN_NM, USER_DEFINED_COLUMN_NM, 
 NBS_RDB_METADATA.BLOCK_PIVOT_NBR, BLOCK_NM, OTHER_VALUE_IND_CD ,
 	         substring(LTRIM(RTRIM(NBS_RDB_METADATA.RDB_COLUMN_NM)),1,22)+'_OTH' as RDB_COLUMN_NM,
...
--and BLOCK_NM != 'REPEATVARCHAR' -- VS 
--ORDER BY RDB_COLUMN_NM
;
 

 --DX  select 'tmp_DynDm_METADATA_UNIT', * from rdb.dbo.tmp_DynDm_METADATA_UNIT;
  ...
  ;
				...
				--select '@countmeta',@countmeta
				 ...
				 select 'No repeat varchar metadata';
				  ...
				  IF OBJECT_ID('rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR', 'U') IS NOT NULL   
 								drop table rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR;
				 ...
				 CREATE TABLE rdb.[dbo].tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR(
						INVESTIGATION_KEY_INVESTIGATION_REPEAT_VARCHAR [bigint] NULL
						) ON [PRIMARY]
					;

					CREATE TABLE rdb.[dbo].tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL(
						INVESTIGATION_KEY_REPEAT_BLOCK_VARCHAR_ALL [bigint] NULL
					) ON [PRIMARY]
...
PROC TRANSPOSE DATA=METADATA OUT=METADATA_OUT;
	VAR USER_DEFINED_COLUMN_NM_1 USER_DEFINED_COLUMN_NM_2 USER_DEFINED_COLUMN_NM_3 USER_DEFINED_COLUMN_NM_4 USER_DEFINED_COLUMN_NM_5;
COPY  BLOCK_NM;
    BY RDB_COLUMN_NM;
RUN;
...
*/
 ...
 IF OBJECT_ID('rdb.dbo.tmp_DynDm_METADATA_OUT', 'U') IS NOT NULL   
 				drop table rdb.dbo.tmp_DynDm_METADATA_OUT;
						...
						--SELECT @columns+=N', p.'+QUOTENAME(LTRIM(RTRIM(USER_DEFINED_COLUMN_NM_1)))
						--FROM
						--(
						--	SELECT USER_DEFINED_COLUMN_NM_1 
						--	FROM rdb.[dbo].tmp_DynDm_METADATA AS p
						--	GROUP BY USER_DEFINED_COLUMN_NM_1
						--	union
						--	SELECT USER_DEFINED_COLUMN_NM_2 
						--	FROM rdb.[dbo].tmp_DynDm_METADATA AS p
						--	GROUP BY USER_DEFINED_COLUMN_NM_2
						...
						--select '@columns0',@columns;
						


						--SET @sql = N'
						--SELECT RDB_COLUMN_NM , block_nm,'+STUFF(@columns, 1, 2, ' ')+
						...
						-- FROM rdb.[dbo].tmp_DynDm_METADATA
						--	group by RDB_COLUMN_NM ,block_nm,USER_DEFINED_COLUMN_NM_1,USER_DEFINED_COLUMN_NM_2,USER_DEFINED_COLUMN_NM_3,USER_DEFINED_COLUMN_NM_4,USER_DEFINED_COLUMN_NM_5
						--		) AS j PIVOT (max(USER_DEFINED_COLUMN_NM_1) FOR USER_DEFINED_COLUMN_NM_1 in 
						...
						--	select 'tmp_DynDm_METADATA_OUT',* from rdb.dbo.tmp_DynDm_METADATA_OUT;
...
FROM rdb.dbo.tmp_DynDm_METADATA_OUT;
 

 --select 'tmp_DynDm_METADATA_OUT_TMP',* from rdb.dbo.tmp_DynDm_METADATA_OUT_TMP;
...
--select 'tmp_DynDm_METADATA_OUT1',* from rdb.dbo.tmp_DynDm_METADATA_OUT1;
...
FROM rdb.dbo.tmp_DynDm_METADATA_OUT_TMP mo
...
--select 'tmp_DynDm_METADATA_OUT_final',* from rdb.dbo.tmp_DynDm_METADATA_OUT_final;
...
--	CREATE TABLE D_INV_REPEAT_METADATA AS 
 ...
 IF OBJECT_ID('rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA', 'U') IS NOT NULL   
	...
	--select 'tmp_DynDm_D_INV_REPEAT_METADATA',* 	from rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA	;
...
*/
 ...
 VALUES ( @batch_id ,'DYNAMIC_DATAMART' ,'RDB.DBO.DynDm_RepeatVarcharData_sp '+@DATAMART_NAME  ,'START' ,@Proc_Step_no ,@Proc_Step_Name ,@ROWCOUNT_NO );
	...
	-- select 'tmp_DynDm_DYNINVLISTING',* from rdb.dbo.tmp_DynDm_DYNINVLISTING;
	...
	--D_INVESTIGATION_REPEAT SELECT '@VARCHAR LIST',@RDB_COLUMN_NAME_LIST;
...
*/
     ...
     IF OBJECT_ID('rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA_distinct', 'U') IS NOT NULL   
 				drop table rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA_distinct;

	select distinct RDB_COLUMN_NM
	into  rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA_distinct
	FROM  rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA;
	...
	FROM  rdb.dbo.tmp_DynDm_D_INV_REPEAT_METADATA_distinct;
	...
	--SELECT @RDB_COLUMN_COMMA_LIST_SELECT          = coalesce((COALESCE(@RDB_COLUMN_COMMA_LIST_SELECT+',' ,'') + RDB_COLUMN_NM + ' AS ' +  coalesce(USER_DEFINED_COLUMN_NM,'')),'')
...
DATA _NULL_;
	CALL SYMPUTX('D_REPEAT_CASE', ''); /*set the value into the first parameter (variable) and LTRIM(  RTRIMs*/
...
--  select 'D_INVESTIGATION_REPEAT', * from rdb.dbo.D_INVESTIGATION_REPEAT;
 ...
 declare @sql varchar(MAX);
  ...
  IF OBJECT_ID('rdb.dbo.D_INVESTIGATION_REPEAT', 'U') IS NOT NULL  
	...
	--declare @RDB_COLUMN_COMMA_LIST_SELECT  varchar(MAX) ='coalesce(EPI_CITY_OF_EXP,'') EPI_CITY_OF_EXP,coalesce(EPI_CNTRY_OF_EXP,'') EPI_CNTRY_OF_EXP,coalesce(EPI_CNTY_OF_EXP,'') EPI_CNTY_OF_EXP,coalesce(EPI_EPI_LEG_ASSTLIVING002,'') EPI_EPI_LEG_ASSTLIVING002,coalesce(EPI_EPI_LEG_ASSTLIVING003,'') EPI_EPI_LEG_ASSTLIVING003,coalesce(EPI_EPI_LEG_ASSTLIVING004,'') EPI_EPI_LEG_ASSTLIVING004,coalesce(EPI_EPI_LEG_ASSTLIVING005,'') EPI_EPI_LEG_ASSTLIVING005,coalesce(EPI_EPI_LEG_ASSTLIVING006,'') EPI_EPI_LEG_ASSTLIVING006,coalesce(EPI_EPI_LEG_HCFVISIT00_OTH,'') EPI_EPI_LEG_HCFVISIT00_OTH,coalesce(EPI_EPI_LEG_HCFVISIT00_OTH,'') EPI_EPI_LEG_HCFVISIT00_OTH,coalesce(EPI_EPI_LEG_HCFVISIT002,'') EPI_EPI_LEG_HCFVISIT002,coalesce(EPI_EPI_LEG_HCFVISIT003,'') EPI_EPI_LEG_HCFVISIT003,coalesce(EPI_EPI_LEG_HCFVISIT004,'') EPI_EPI_LEG_HCFVISIT004,coalesce(EPI_EPI_LEG_HCFVISIT005,'') EPI_EPI_LEG_HCFVISIT005,coalesce(EPI_EPI_LEG_HCFVISIT006,'') EPI_EPI_LEG_HCFVISIT006,coalesce(EPI_EPI_LEG_HCFVISIT007,'') EPI_EPI_LEG_HCFVISIT007,coalesce(EPI_EPI_LEG_HCFVISIT008,'') EPI_EPI_LEG_HCFVISIT008,coalesce(EPI_EPI_LEG_HOTEL002,'') EPI_EPI_LEG_HOTEL002,coalesce(EPI_EPI_LEG_HOTEL003,'') EPI_EPI_LEG_HOTEL003,coalesce(EPI_EPI_LEG_HOTEL004,'') EPI_EPI_LEG_HOTEL004,coalesce(EPI_EPI_LEG_HOTEL005,'') EPI_EPI_LEG_HOTEL005,coalesce(EPI_EPI_LEG_HOTEL006,'') EPI_EPI_LEG_HOTEL006,coalesce(EPI_EPI_LEG_HOTEL007,'') EPI_EPI_LEG_HOTEL007,coalesce(EPI_EPI_LEG_HOTEL008,'') EPI_EPI_LEG_HOTEL008,coalesce(EPI_ST_OR_PROV_OF_EXP,'') EPI_ST_OR_PROV_OF_EXP,coalesce(LAB_PERFORMING_LAB_TYPE,'') LAB_PERFORMING_LAB_TYPE,coalesce(LAB_SEROGROUP,'') LAB_SEROGROUP,coalesce(LAB_SPECIES,'') LAB_SPECIES,coalesce(LAB_SPECIMEN_TYPE,'') LAB_SPECIMEN_TYPE,coalesce(LAB_TEST_RESULT,'') LAB_TEST_RESULT,coalesce(LAB_TEST_RESULT_OTH,'') LAB_TEST_RESULT_OTH,coalesce(LAB_TEST_TYPE,'') LAB_TEST_TYPE,coalesce(LAB_TEST_TYPE_OTH,'') LAB_TEST_TYPE_OTH,coalesce(TRT_TREATMENT_ABX002,'') TRT_TREATMENT_ABX002,coalesce(TRT_TREATMENT_ABX002_OTH,'') TRT_TREATMENT_ABX002_OTH,coalesce(TRV_TRAVEL_OOC002,'') TRV_TRAVEL_OOC002,coalesce(TRV_TRAVEL_OOC002_OTH,'') TRV_TRAVEL_OOC002_OTH,coalesce(TRV_TRAVEL_OOC005,'') TRV_TRAVEL_OOC005,coalesce(TRV_TRAVEL_OOCO002,'') TRV_TRAVEL_OOCO002,coalesce(TRV_TRAVEL_OOCO002_OTH,'') TRV_TRAVEL_OOCO002_OTH,coalesce(TRV_TRAVEL_OOCO005,'') TRV_TRAVEL_OOCO005,coalesce(TRV_TRAVEL_OOS002,'') TRV_TRAVEL_OOS002,coalesce(TRV_TRAVEL_OOS002_OTH,'') TRV_TRAVEL_OOS002_OTH,coalesce(TRV_TRAVEL_OOS005,'') TRV_TRAVEL_OOS005 ';
	 ...
	 SET @SQL = '   SELECT '+ @RDB_COLUMN_COMMA_LIST_SELECT + '  ,ANSWER_GROUP_SEQ_NBR, D_INVESTIGATION_REPEAT.D_INVESTIGATION_REPEAT_KEY, tsd.INVESTIGATION_KEY, D_INVESTIGATION_REPEAT.BLOCK_NM ' +
				...
				' FROM rdb.dbo.tmp_DynDm_SUMM_DATAMART tsd ' + 
				'  INNER JOIN  rdb.dbo.'+ @FACT_CASE+ '  ON	tsd.INVESTIGATION_KEY = '+ @FACT_CASE+'.INVESTIGATION_KEY ' + 
	 ...
	 --select 'tmp_DynDm_REPEAT_BLOCK',@SQL;
 ...
 --DX  select '@RDB_COLUMN_COMMA_LIST',@RDB_COLUMN_COMMA_LIST;
...
PROC TRANSPOSE DATA=REPEAT_BLOCK OUT=REPEAT_BLOCK_OUT;
	...
	VAR &D_REPEAT_CASE_NAME; -- @RDB_COLUMN_LIST
RUN;
*/
 ...
 VALUES ( @batch_id ,'DYNAMIC_DATAMART' ,'RDB.DBO.DynDm_RepeatVarcharData_sp '+@DATAMART_NAME  ,'START' ,@Proc_Step_no ,@Proc_Step_Name ,@ROWCOUNT_NO );
	...
	SET @Proc_Step_Name = 'GENERATING  tmp_DynDm_REPEAT_BLOCK_OUT';
     ...
     IF OBJECT_ID('rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT', 'U') IS NOT NULL   
				...
				-- select * from tmp_DynDm_REPEAT_BLOCK
---VS--********
					...
					-- select '@columns_1', @columns;
						...
						SET @sql = N' select INVESTIGATION_KEY, BLOCK_NM BLOCK_NM_REPEAT_BLOCK_OUT, ANSWER_GROUP_SEQ_NBR ANSWER_GROUP_SEQ_NBR_REPEAT_BLOCK_OUT,variable as RDB_COLUMN_NM_REPEAT_BLOCK_OUT,value as COL1 '+
						  ...
						  ' from  rdb.dbo.tmp_DynDm_REPEAT_BLOCK '+
						  ' ) as t '+
						  ' unpivot ( '+
...
---VS--********
						...
						--select 'tmp_DynDm_REPEAT_BLOCK',* from rdb.dbo.tmp_DynDm_REPEAT_BLOCK;
			--	select 'tmp_DynDm_REPEAT_BLOCK_OUT',* from rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT;
...
--DX select 'rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT', * from rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT;


--/*2 identical tables with different names*/
 
----CREATE TABLE BLOCK_DATA_PL AS	
...
-- select 'tmp_DynDm_BLOCK_DATA_PL',* from  rdb.dbo.tmp_DynDm_BLOCK_DATA_PL;
...
-- select 'tmp_DynDm_BLOCK_DATA_UNIT', * from rdb.dbo.tmp_DynDm_BLOCK_DATA_UNIT;
...
--select 'tmp_DynDm_BLOCK_DATA_OTH', * from rdb.dbo.tmp_DynDm_BLOCK_DATA_OTH;
...
FROM rdb.dbo.tmp_DynDm_BLOCK_DATA_PL
UNION SELECT * FROM rdb.dbo.tmp_DynDm_BLOCK_DATA_UNIT
UNION SELECT * FROM rdb.dbo.tmp_DynDm_BLOCK_DATA_OTH;
...
FROM rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT bo
	INNER JOIN rdb.dbo.tmp_DynDm_BLOCK_DATA bd ON 	bo.BLOCK_NM_REPEAT_BLOCK_OUT =bd.BLOCK_NM
...
-- select 'tmp_DynDm_REPEAT_BLOCK_OUT_BASE',* from rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_BASE
...
----VS temp ANSWER_DESC21
-- select * , cast(COL1 as varchar(8000)) as ANSWER_DESC21
...
into rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_ALL
FROM rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_BASE AS a
...
-- select 'tmp_DynDm_REPEAT_BLOCK_OUT_ALL',* from rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_ALL;
...
FROM rdb.dbo.tmp_DynDm_METADATA_OUT_final;
 
 --DX select 'tmp_DynDm_METADATA_OUT2',* from rdb.dbo.tmp_DynDm_METADATA_OUT2;
...
FROM rdb.dbo.tmp_DynDm_METADATA_OUT_final tdof 
LEFT OUTER JOIN rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_BASE  trbb ON UPPER(tdof.RDB_COLUMN_NM)  = UPPER(trbb.RDB_COLUMN_NM_REPEAT_BLOCK_OUT)
 ...
 VALUES ( @batch_id ,'DYNAMIC_DATAMART' ,'RDB.DBO.DynDm_RepeatVarcharData_sp '+@DATAMART_NAME  ,'START' ,@Proc_Step_no ,@Proc_Step_Name ,@ROWCOUNT_NO );
	...
	SET @Proc_Step_Name = 'GENERATING  tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR';
			...
			IF OBJECT_ID('rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR', 'U') IS NOT NULL 
						...
						(
							SELECT [COL1] 
							FROM rdb.[dbo].tmp_DynDm_METADATA_MERGED_INIT AS p
							GROUP BY [COL1]
						) AS x;
						...
						' into rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR ' +
						'FROM (
						...
						--DX select 'tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR',* from rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR;
...
--DX select * from rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_ALL;

--DX select * from rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_ALL
...
FROM rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_ALL  ra
LEFT OUTER JOIN rdb.dbo.tmp_DynDm_REPEAT_BLOCK_OUT_ALL  rboa ON UPPER(ra.RDB_COLUMN_NM) = UPPER(rboa.RDB_COLUMN_NM_REPEAT_BLOCK_OUT);
 

-- select ' rdb.dbo.tmp_DynDm_REPEAT_BLOCK_METADATA_OUT',* from rdb.dbo.tmp_DynDm_REPEAT_BLOCK_METADATA_OUT;
	...
	SET @Proc_Step_Name = 'GENERATING  tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL';
			...
			IF OBJECT_ID('rdb.dbo.tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL', 'U') IS NOT NULL 
						...
						(
							...
							FROM rdb.[dbo].tmp_DynDm_REPEAT_BLOCK_METADATA_OUT AS p
						...
						SET @sql = N'
						SELECT [INVESTIGATION_KEY] , '+STUFF(@columns, 1, 2, '')+
						' into rdb.dbo.tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL ' +
						'FROM (
...
--DX select 'tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL',* from tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL;

exec sp_rename 'rdb.dbo.tmp_DynDm_INVESTIGATION_REPEAT_VARCHAR.INVESTIGATION_KEY', 'INVESTIGATION_KEY_INVESTIGATION_REPEAT_VARCHAR','COLUMN';
exec sp_rename 'rdb.dbo.tmp_DynDm_REPEAT_BLOCK_VARCHAR_ALL.INVESTIGATION_KEY', 'INVESTIGATION_KEY_REPEAT_BLOCK_VARCHAR_ALL','COLUMN';
...
--*/
    ...
    END TRY
            BEGIN CATCH
						...
						VALUES( @Batch_id, 'DYNAMIC_DATAMART', 'RDB.DBO.DynDm_RepeatVarcharData_sp ' + @DATAMART_NAME, 'ERROR', @Proc_Step_no, 'ERROR - '+@Proc_Step_name, 'Step -'+CAST(@Proc_Step_no AS varchar(3))+' -'+CAST(@ErrorMessage AS varchar(500)), 0 );
						RETURN -1;
	        END CATCH;
END;
