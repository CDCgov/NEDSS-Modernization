...
IF OBJECT_ID('dbo.sp_LDF_MUMPS_DATAMART', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].sp_LDF_MUMPS_DATAMART;
GO

CREATE PROCEDURE [dbo].sp_LDF_MUMPS_DATAMART
  @batch_id BIGINT
 as
	...
	BEGIN
			...
			BEGIN TRANSACTION;
				...
				INSERT INTO RDB.[DBO].[JOB_FLOW_LOG] 
					...
					VALUES(@BATCH_ID,'LDF_MUMPS','RDB.LDF_MUMPS','START',@PROC_STEP_NO,@PROC_STEP_NAME,@ROWCOUNT_NO);  
				...
				IF OBJECT_ID('RDB.dbo.TMP_LINKED_MUMPS', 'U') IS NOT NULL  
						...
						INTO  RDB.dbo.TMP_LINKED_MUMPS
						FROM
							RDB.dbo.TMP_BASE_MUMPS GEN_LDF with (nolock)
			...
			-- CREATE TABLE ALL_MUMPS
				...
				IF OBJECT_ID('RDB.dbo.TMP_ALL_MUMPS_SHORT_COL', 'U') IS NOT NULL  
						...
						SELECT INVESTIGATION_KEY,
								...
								DATAMART_COLUMN_NM,
								col1  
						INTO RDB.dbo.TMP_ALL_MUMPS_SHORT_COL
						FROM RDB.dbo.TMP_ALL_MUMPS with (nolock) 
				...
				SET @PROC_STEP_NAME = ' GENERATING TMP_ALL_MUMPS_TA'; 
					...
					IF OBJECT_ID('RDB.dbo.TMP_ALL_MUMPS_TA', 'U') IS NOT NULL  
						...
						SELECT INVESTIGATION_KEY,
								...
								DATAMART_COLUMN_NM,
								col1  
						INTO RDB.dbo.TMP_ALL_MUMPS_TA
						FROM RDB.dbo.TMP_ALL_MUMPS 
				...
				BEGIN
					IF OBJECT_ID('RDB.dbo.TMP_MUMPS_TA', 'U') IS NOT NULL  
							...
							ALTER TABLE TMP_ALL_MUMPS_TA
							ADD ANSWERCOL varchar(8000);

							UPDATE TMP_ALL_MUMPS_TA
				...
				set @count = (SELECT count(*) FROM RDB.dbo.TMP_ALL_MUMPS_TA);
				IF @count=0
				BEGIN
					IF OBJECT_ID('RDB.dbo.TMP_MUMPS_TA', 'U') IS NOT NULL  
						DROP TABLE RDB.dbo.TMP_MUMPS_TA;
						...
						SELECT INVESTIGATION_KEY,
							   ...
							   DISEASE_NAME,
							   DISEASE_CD
						INTO RDB.dbo.TMP_MUMPS_TA
						FROM RDB.dbo.TMP_ALL_MUMPS_TA with (nolock);
				...
				set @count = (SELECT count(*) FROM TMP_ALL_MUMPS_SHORT_COL)
					...
					BEGIN
						...
						IF OBJECT_ID('RDB.dbo.TMP_MUMPS_SHORT_COL', 'U') IS NOT NULL  
							...
							ALTER TABLE RDB.dbo.TMP_ALL_MUMPS_SHORT_COL
							ADD ANSWERCOL varchar(8000);

							UPDATE RDB.dbo.TMP_ALL_MUMPS_SHORT_COL
							SET ANSWERCOL = SUBSTRING(COL1, 1, 8000); 
							...
							SET @query='';
							BEGIN TRY
								SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct DATAMART_COLUMN_NM from RDB.dbo.TMP_ALL_MUMPS_SHORT_COL ) as tmp
								select @cols = substring(@cols, 0, len(@cols)) --trim "," at end

								set @query = 
								'SELECT *
								INTO TMP_MUMPS_SHORT_COL
								FROM
								( 
								SELECT     INVESTIGATION_KEY,
										   INVESTIGATION_LOCAL_ID,
								...
								FROM RDB.dbo.TMP_ALL_MUMPS_SHORT_COL )
								as A 
							...
							BEGIN CATCH
								...
								BEGIN
									SET @cols='';
									SET @query='';
									SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct top 300 DATAMART_COLUMN_NM from RDB.dbo.TMP_ALL_MUMPS_SHORT_COL ) as tmp
									select @cols = substring(@cols, 0, len(@cols)) --trim "," at end
				...
				set @count = (SELECT count(*) FROM RDB.dbo.TMP_ALL_MUMPS_SHORT_COL)
				IF @count=0
				BEGIN
				IF OBJECT_ID('RDB.dbo.TMP_MUMPS_SHORT_COL', 'U') IS NOT NULL  
						DROP TABLE RDB.dbo.TMP_MUMPS_SHORT_COL;
					...
					SELECT INVESTIGATION_KEY,
						   ...
						   DISEASE_NAME,
						   DISEASE_CD
						INTO RDB.dbo.TMP_MUMPS_SHORT_COL
						FROM RDB.dbo.TMP_ALL_MUMPS_SHORT_COL with (nolock);
				END	
				...
				SET @PROC_STEP_NAME = ' GENERATING TMP_MUMPS'; 
					...
					IF OBJECT_ID('RDB.dbo.TMP_MUMPS', 'U') IS NOT NULL  
						...
						EXECUTE  [dbo].[sp_MERGE_TWO_TABLES] 
						   @INPUT_TABLE1='RDB.dbo.TMP_MUMPS_SHORT_COL'
						  ,@INPUT_TABLE2='RDB.dbo.TMP_MUMPS_TA'
						  ,@OUTPUT_TABLE='RDB.dbo.TMP_MUMPS'
						  ,@JOIN_ON_COLUMN='INVESTIGATION_KEY';

						SELECT @ROWCOUNT_NO = @@ROWCOUNT;
					
						DELETE FROM RDB.dbo.TMP_MUMPS WHERE INVESTIGATION_KEY IS NULL;
				...
				SET @PROC_STEP_NAME = ' GENERATING LDF_MUMPS'; 
					...
					IF OBJECT_ID('RDB.dbo.LDF_MUMPS', 'U') IS NOT NULL
					BEGIN
						BEGIN TRANSACTION;
							SET @Alterdynamiccolumnlist='';
							SET @dynamiccolumnUpdate='';
							 
							SELECT   @Alterdynamiccolumnlist  = @Alterdynamiccolumnlist+ 'ALTER TABLE RDB.dbo.LDF_MUMPS ADD [' + name   +  '] varchar(4000) ',
								@dynamiccolumnUpdate= @dynamiccolumnUpdate + 'LDF_MUMPS.[' +  name  + ']='  + 'RDB.dbo.TMP_MUMPS.['  +name  + '] ,'
							FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB.dbo.TMP_MUMPS')
							AND name NOT IN  ( SELECT name FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB..LDF_MUMPS'))
							...
							BEGIN
								...
								EXEC ('update  RDB.dbo.LDF_MUMPS  SET ' +   @dynamiccolumnUpdate + ' FROM RDB.dbo.TMP_MUMPS     
								   inner join  RDB.dbo.LDF_MUMPS  on  RDB.dbo.TMP_MUMPS.INVESTIGATION_LOCAL_ID =  RDB.dbo.LDF_MUMPS.INVESTIGATION_LOCAL_ID')
						...
						BEGIN TRANSACTION;
							...
							DELETE FROM RDB.dbo.LDF_MUMPS WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM RDB.dbo.TMP_MUMPS);
						...
						--- During update if TMP_MUMPS has 4 columns updated only and the LDF_MUMPS has 7 columns then get column name dynamically from TMP_MUMPS and populate them.
							...
							SELECT @dynamiccolumnList= @dynamiccolumnList +'['+ name +'],' FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB.dbo.TMP_MUMPS')
							...
							EXEC ('INSERT INTO RDB.dbo.LDF_MUMPS ('+@dynamiccolumnList+')
							SELECT '+@dynamiccolumnList +'
							FROM RDB.dbo.TMP_MUMPS');
					...
					IF OBJECT_ID('RDB.dbo.LDF_MUMPS', 'U') IS NULL 
					BEGIN
						BEGIN TRANSACTION;
						
							SELECT *
							INTO RDB.dbo.LDF_MUMPS
							FROM RDB.dbo.TMP_MUMPS;
			...
			END	-- END for IF EXISTS (SELECT * INTO RDB.dbo.TMP_BASE_MUMPS WHERE  PHC_CD IN .................  at line 69
