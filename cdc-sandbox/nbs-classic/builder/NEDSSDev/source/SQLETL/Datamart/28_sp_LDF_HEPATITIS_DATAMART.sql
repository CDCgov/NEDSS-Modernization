...
IF OBJECT_ID('dbo.sp_LDF_HEPATITIS_DATAMART', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].sp_LDF_HEPATITIS_DATAMART;
GO
 ...
 BEGIN TRY
	...
	BEGIN
		...
		BEGIN TRANSACTION;
			...
			INTO RDB.dbo.TMP_BASE_HEPATITIS
			FROM RDB.dbo.S_LDF_DIMENSIONAL_DATA
		    ...
		    INSERT INTO RDB.[DBO].[JOB_FLOW_LOG] 
				...
				VALUES(@BATCH_ID,'LDF_HEPATITIS','RDB.LDF_HEPATITIS','START',@PROC_STEP_NO,@PROC_STEP_NAME,@ROWCOUNT_NO);  
			...
			IF OBJECT_ID('RDB.dbo.TMP_ALL_HEPATITIS', 'U') IS NOT NULL  
					...
					INTO RDB.dbo.TMP_ALL_HEPATITIS
					FROM RDB.dbo.LDF_DATAMART_COLUMN_REF  B 
					FULL OUTER JOIN RDB.dbo.TMP_LINKED_HEPATITIS A 
					ON A.LDF_UID= B.LDF_UID WHERE
					(B.LDF_PAGE_SET ='HEP'
					OR B.CONDITION_CD IN (SELECT CONDITION_CD FROM 
						RDB.dbo.LDF_DATAMART_TABLE_REF WHERE DATAMART_NAME = 'LDF_HEPATITIS') 
					)
					ORDER BY INVESTIGATION_UID;

					UPDATE RDB.dbo.TMP_ALL_HEPATITIS
			...
			IF OBJECT_ID('RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL', 'U') IS NOT NULL  
					...
					SELECT INVESTIGATION_KEY,
							...
							DATAMART_COLUMN_NM,
							col1
					INTO RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL
					FROM RDB.dbo.TMP_ALL_HEPATITIS 
			...
			SET @PROC_STEP_NAME = ' GENERATING TMP_ALL_HEPATITIS_TA'; 
				...
				IF OBJECT_ID('RDB.dbo.TMP_ALL_HEPATITIS_TA', 'U') IS NOT NULL  
					...
					SELECT INVESTIGATION_KEY,
								   ...
								   DATAMART_COLUMN_NM,
								    col1
					INTO RDB.dbo.TMP_ALL_HEPATITIS_TA
					FROM RDB.dbo.TMP_ALL_HEPATITIS 
			...
			BEGIN
				IF OBJECT_ID('RDB.dbo.TMP_HEPATITIS_TA', 'U') IS NOT NULL  
						...
						FROM RDB.dbo.TMP_ALL_HEPATITIS_TA )
						as A 
			...
			set @count = (SELECT count(*) FROM RDB.dbo.TMP_ALL_HEPATITIS_TA)
			IF @count=0
			BEGIN
				IF OBJECT_ID('RDB.dbo.TMP_HEPATITIS_TA', 'U') IS NOT NULL  
					DROP TABLE RDB.dbo.TMP_HEPATITIS_TA;
			...
			set @count = (SELECT count(*) FROM RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL)
			IF @count>0
				BEGIN
					IF OBJECT_ID('RDB.dbo.TMP_HEPATITIS_SHORT_COL', 'U') IS NOT NULL  
						 DROP TABLE RDB.dbo.TMP_HEPATITIS_SHORT_COL;
						...
						ALTER TABLE RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL
						ADD ANSWERCOL varchar(8000);

						UPDATE RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL
						SET ANSWERCOL = SUBSTRING(COL1, 1, 8000); 
						...
						SET @query='';
						BEGIN TRY
							SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct DATAMART_COLUMN_NM from RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL ) as tmp
							select @cols = substring(@cols, 0, len(@cols)) --trim "," at end
							...
							FROM RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL )
							as A 
						...
						BEGIN CATCH
							...
							BEGIN
								SET @cols='';
								SET @query='';
								SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct top 300 DATAMART_COLUMN_NM from RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL ) as tmp
								select @cols = substring(@cols, 0, len(@cols)) --trim "," at end
			...
			IF OBJECT_ID('RDB.dbo.TMP_HEPATITIS_SHORT_COL', 'U') IS NOT NULL  
				...
				SELECT INVESTIGATION_KEY,
					   ...
					   DISEASE_NAME,
					   DISEASE_CD
					INTO RDB.dbo.TMP_HEPATITIS_SHORT_COL
					FROM RDB.dbo.TMP_ALL_HEPATITIS_SHORT_COL;
			END	
			...
			SET @PROC_STEP_NAME = ' GENERATING TMP_HEPATITIS'; 
				...
				EXECUTE  [dbo].[sp_MERGE_TWO_TABLES] 
						   @INPUT_TABLE1='RDB.dbo.TMP_HEPATITIS_SHORT_COL'
						  ,@INPUT_TABLE2='RDB.dbo.TMP_HEPATITIS_TA'
						  ,@OUTPUT_TABLE='RDB.dbo.TMP_HEPATITIS'
						  ,@JOIN_ON_COLUMN='INVESTIGATION_KEY';

					SELECT @ROWCOUNT_NO = @@ROWCOUNT;
					
					DELETE FROM RDB.dbo.TMP_HEPATITIS WHERE INVESTIGATION_KEY IS NULL;
			...
			SET @PROC_STEP_NAME = ' GENERATING LDF_HEPATITIS'; 
				IF OBJECT_ID('RDB.dbo.LDF_HEPATITIS', 'U') IS NOT NULL
				BEGIN
					BEGIN TRANSACTION;
						SET @Alterdynamiccolumnlist='';
						SET @dynamiccolumnUpdate='';						 
						SELECT   @Alterdynamiccolumnlist  = @Alterdynamiccolumnlist+ 'ALTER TABLE RDB.dbo.LDF_HEPATITIS ADD [' + name   +  '] varchar(4000) ',
							@dynamiccolumnUpdate= @dynamiccolumnUpdate + 'LDF_HEPATITIS.[' +  name  + ']='  + 'RDB.dbo.TMP_HEPATITIS.['  +name  + '] ,'
						FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB.dbo.TMP_HEPATITIS')
						AND name NOT IN  ( SELECT name FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB..LDF_HEPATITIS'));
						...
						BEGIN
							...
							EXEC ('update  RDB.dbo.LDF_HEPATITIS  SET ' +   @dynamiccolumnUpdate + ' FROM RDB.dbo.TMP_HEPATITIS     
							   inner join  RDB.dbo.LDF_HEPATITIS  on  RDB.dbo.TMP_HEPATITIS.INVESTIGATION_LOCAL_ID =  RDB.dbo.LDF_HEPATITIS.INVESTIGATION_LOCAL_ID');
					...
					BEGIN TRANSACTION;
						...
						DELETE FROM RDB.dbo.LDF_HEPATITIS WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM RDB.dbo.TMP_HEPATITIS);
					...
					--- During update if TMP_HEPATITIS has 4 columns updated only and the LDF_HEPATITIS has 7 columns then get column name dynamically from TMP_HEPATITIS and populate them.
						...
						SELECT @dynamiccolumnList= @dynamiccolumnList +'['+ name +'],' FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB.dbo.TMP_HEPATITIS');
						...
						EXEC ('INSERT INTO RDB.dbo.LDF_HEPATITIS ('+@dynamiccolumnList+')
						SELECT '+@dynamiccolumnList +'
						FROM RDB.dbo.TMP_HEPATITIS');
				...
				IF OBJECT_ID('RDB.dbo.LDF_HEPATITIS', 'U') IS NULL 
				BEGIN
					BEGIN TRANSACTION;
					
						SELECT *
						INTO RDB.dbo.LDF_HEPATITIS
						FROM RDB.dbo.TMP_HEPATITIS;
