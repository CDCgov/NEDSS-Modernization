...
IF OBJECT_ID('dbo.sp_LDF_BMIRD_DATAMART', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].sp_LDF_BMIRD_DATAMART;
GO
		...
		BEGIN
			...
			BEGIN TRANSACTION;
				...
				INSERT INTO RDB.[DBO].[JOB_FLOW_LOG] 
					...
					VALUES(@BATCH_ID,'LDF_BMIRD','RDB.LDF_BMIRD','START',@PROC_STEP_NO,@PROC_STEP_NAME,@ROWCOUNT_NO);  
				...
				IF OBJECT_ID('RDB.dbo.TMP_LINKED_BMIRD', 'U') IS NOT NULL  
						...
						INTO  RDB.dbo.TMP_LINKED_BMIRD
						FROM
							RDB.dbo.TMP_BASE_BMIRD GEN_LDF with (nolock)
							INNER JOIN  RDB.dbo.INVESTIGATION INV
						ON  
							GEN_LDF.INVESTIGATION_UID=INV.CASE_UID 
						INNER JOIN RDB.dbo.BMIRD_CASE GEN with (nolock)
						ON 
			...
			-- CREATE TABLE ALL_BMIRD
				...
				IF OBJECT_ID('RDB.dbo.TMP_ALL_BMIRD_SHORT_COL', 'U') IS NOT NULL  
						...
						SELECT INVESTIGATION_KEY,
								...
								DATAMART_COLUMN_NM,
								col1 
						INTO RDB.dbo.TMP_ALL_BMIRD_SHORT_COL
						FROM RDB.dbo.TMP_ALL_BMIRD with (nolock) 
				...
				set @count = (SELECT count(*) FROM RDB.dbo.TMP_ALL_BMIRD_TA)
				IF @count = 0
				BEGIN
					IF OBJECT_ID('RDB.dbo.TMP_BMIRD_TA', 'U') IS NOT NULL  
						DROP TABLE RDB.dbo.TMP_BMIRD_TA;
						...
						SELECT INVESTIGATION_KEY,
							   ...
							   DISEASE_NAME,
							   DISEASE_CD
						INTO RDB.dbo.TMP_BMIRD_TA
						FROM RDB.dbo.TMP_ALL_BMIRD_TA with (nolock);
				...
				IF @count > 0
					BEGIN
						...
						IF OBJECT_ID('RDB.dbo.TMP_BMIRD_SHORT_COL', 'U') IS NOT NULL  
							...
							ALTER TABLE RDB.dbo.TMP_ALL_BMIRD_SHORT_COL
							ADD ANSWERCOL varchar(8000);

							UPDATE RDB.dbo.TMP_ALL_BMIRD_SHORT_COL
							SET ANSWERCOL = SUBSTRING(COL1, 1, 8000);
							...
							SET @query='';
							BEGIN TRY
								SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct DATAMART_COLUMN_NM from RDB.dbo.TMP_ALL_BMIRD_SHORT_COL ) as tmp
								select @cols = substring(@cols, 0, len(@cols)) --trim "," at end
								...
								FROM RDB.dbo.TMP_ALL_BMIRD_SHORT_COL with (nolock) )
								as A 
							...
							BEGIN CATCH
								...
								BEGIN
									SET @cols='';
									SET @query='';
									SELECT @cols = @cols + QUOTENAME(DATAMART_COLUMN_NM) + ',' FROM (select distinct top 300 DATAMART_COLUMN_NM from RDB.dbo.TMP_ALL_BMIRD_SHORT_COL ) as tmp
									select @cols = substring(@cols, 0, len(@cols)) --trim "," at end
				...
				IF OBJECT_ID('RDB.dbo.TMP_BMIRD_SHORT_COL', 'U') IS NOT NULL  
					...
					SELECT INVESTIGATION_KEY,
						   ...
						   DISEASE_NAME,
						   DISEASE_CD
						INTO RDB.dbo.TMP_BMIRD_SHORT_COL
						FROM RDB.dbo.TMP_ALL_BMIRD_SHORT_COL with (nolock);
				END	
				...
				SET @PROC_STEP_NAME = ' GENERATING TMP_BMIRD'; 
					...
					IF OBJECT_ID('RDB.dbo.TMP_BMIRD', 'U') IS NOT NULL  
						...
						EXECUTE  [dbo].[sp_MERGE_TWO_TABLES] 
						   @INPUT_TABLE1='RDB.dbo.TMP_BMIRD_SHORT_COL'
						  ,@INPUT_TABLE2='RDB.dbo.TMP_BMIRD_TA'
						  ,@OUTPUT_TABLE='RDB.dbo.TMP_BMIRD'
						  ,@JOIN_ON_COLUMN='INVESTIGATION_KEY';

						SELECT @ROWCOUNT_NO = @@ROWCOUNT;
					
						DELETE FROM RDB.dbo.TMP_BMIRD WHERE INVESTIGATION_KEY IS NULL;
				...
				SET @PROC_STEP_NAME = ' GENERATING LDF_BMIRD'; 
					...
					IF OBJECT_ID('RDB.dbo.LDF_BMIRD', 'U') IS NOT NULL
					BEGIN
						BEGIN TRANSACTION;
							SET @Alterdynamiccolumnlist='';
							SET @dynamiccolumnUpdate='';
							 
							SELECT   @Alterdynamiccolumnlist  = @Alterdynamiccolumnlist+ 'ALTER TABLE RDB.dbo.LDF_BMIRD ADD [' + name   +  '] varchar(4000) ',
								@dynamiccolumnUpdate= @dynamiccolumnUpdate + 'LDF_BMIRD.[' +  name  + ']='  + 'RDB.dbo.TMP_BMIRD.['  +name  + '] ,'
							FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB.dbo.TMP_BMIRD')
							AND name NOT IN  ( SELECT name FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB..LDF_BMIRD'));
							...
							BEGIN
								...
								EXEC ('update  RDB.dbo.LDF_BMIRD  SET ' +   @dynamiccolumnUpdate + ' FROM RDB.dbo.TMP_BMIRD     
								   inner join  RDB.dbo.LDF_BMIRD  on  RDB.dbo.TMP_BMIRD.INVESTIGATION_LOCAL_ID =  RDB.dbo.LDF_BMIRD.INVESTIGATION_LOCAL_ID');
						...
						BEGIN TRANSACTION;
							...
							DELETE FROM RDB.dbo.LDF_BMIRD WHERE INVESTIGATION_KEY IN (SELECT INVESTIGATION_KEY FROM RDB.dbo.TMP_BMIRD);
						...
						--- During update if TMP_BMIRD has 4 columns updated only and the LDF_BMIRD has 7 columns then get column name dynamically from TMP_BMIRD and populate them.
							...
							SELECT @dynamiccolumnList= @dynamiccolumnList +'['+ name +'],' FROM  RDB.Sys.Columns WHERE Object_ID = Object_ID('RDB.dbo.TMP_BMIRD');
							...
							EXEC ('INSERT INTO RDB.dbo.LDF_BMIRD ('+@dynamiccolumnList+')
							SELECT '+@dynamiccolumnList +'
							FROM RDB.dbo.TMP_BMIRD');
					...
					IF OBJECT_ID('RDB.dbo.LDF_BMIRD', 'U') IS NULL 
					BEGIN
						BEGIN TRANSACTION;
						
							SELECT *
							INTO RDB.dbo.LDF_BMIRD
							FROM RDB.dbo.TMP_BMIRD;
			...
			END	-- END for IF EXISTS (SELECT * INTO RDB.dbo.TMP_BASE_BMIRD WHERE  PHC_CD IN .................  at line 69
