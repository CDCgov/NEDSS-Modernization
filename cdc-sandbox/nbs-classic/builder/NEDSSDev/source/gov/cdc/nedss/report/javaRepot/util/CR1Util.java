...
/**
 * Util class for Chalk report 1
 * @author Pradeep Kuamr Sharma
 *
 */
public class CR1Util {
	...
	static final LogUtils logger = new LogUtils(CR1Util.class.getName());
		...
		try {
			...
			if(coll!=null && coll.size()>0){
				...
				while(iterator.hasNext()){
					...
					Collection treatments =treatmentDAO.getTreatments(investigationKey);
					...
					cr1VO.setTreatments(treatments);
					...
					if(socialAssociatesMap.containsKey(investigationKey)){
						...
						reportPatientDT.setSocialANAssociateContacts(partnerRelationshipCollection.size()+"");
					}else{
