...
/**
 *
 * <p>Title: CrsCreateLoad</p>
 * <p>Description: This is a load action class for create investigation for the Congenital Rubella Syndrome conditions.</p>
...
public class CrsCreateLoad
    extends BaseCreateLoad
{

    static final LogUtils logger = new LogUtils(CrsCreateLoad.class.getName());
    /**
     * empty constructor
     */
    public CrsCreateLoad()
    {
    }
    /**
    ...
    */
	...
	try{ 
		...
		// prepareLDF(investigationForm.getProxy(), request);

		if (investigationFormCd == null)
		{
			logger.error(" could not get investigationFormCd");
			session.setAttribute("error", "ConditionCd or InvestigationFormCd is null");

			return (mapping.findForward("error"));
		}

		//gov.cdc.nedss.utils.VOTester.createReport(investigationForm, "create-load");
		...
		if(sCurrentTask.equals("CreateInvestigation5") || sCurrentTask.equals("CreateInvestigation6") || sCurrentTask.equals("CreateInvestigation7")|| sCurrentTask.equals("CreateInvestigation8"))
		{
			...
			TreeMap<Object,Object> morbMap = (TreeMap<Object, Object>)NBSContext.retrieve(session, NBSConstantUtil.DSMorbMap);
			super.prepopulateMorbValues(morbMap, request);
			String INV145 = (String)morbMap.get("INV145");
			if(INV145 != null && INV145.equals(NEDSSConstants.YES))
				request.setAttribute("CRS180", INV145);
		}
		...
		logger.error("Exception in CRS Load: " + e.getMessage());
		e.printStackTrace();
		throw new ServletException("Error occurred during CRS Load : "+e.getMessage());
	}
	...
	String path = "/error";
        ...
        if (NBSConstantUtil.INV_FORM_CRS.equals(investigationFormCd)&& viewContactTracing)
	{
	    path = "/diseaseform/nip/crs/investigation_crs_create?CurrentTask=" + sCurrentTask +
	...
	}else if (NBSConstantUtil.INV_FORM_CRS.equals(investigationFormCd)&& !viewContactTracing)
	{
	    path = "/diseaseform/nip/crs/investigation_crs_create_No_Contact?CurrentTask=" + sCurrentTask +
		   "&ContextAction=" + sContextAction;
	    CRSInvestigationUtil crsInvestigationUtil = new CRSInvestigationUtil();
	    crsInvestigationUtil.getVaccinationSummaryRecords(mprUid, request);
	}
