...
public class InvestigationViewLoad extends CommonAction
{

    //For logging
    static final LogUtils logger = new LogUtils(InvestigationViewLoad.class.getName());

     /**
       * This is constructor
       *
       */
    public InvestigationViewLoad()
    {
    }
    ...
    {       
    	...
    	/*String businessType = (String)request.getAttribute("BusinessObjectType");
    	
		String formCd = (String)request.getAttribute("PageFormCd");
		String mode = (String)request.getAttribute("mode");
		if(businessType!=null && businessType.equalsIgnoreCase(NEDSSConstants.ISOLATE_BUSINESS_OBJECT_TYPE)){
			if(mode!=null && mode=="Delete")
				return mapping.findForward("DMBViewForm");
			else
				return mapping.findForward(this.getActionForward(formCd+"_page", request));
        ...
        try{
			...
			e.printStackTrace();
			logger.error("InvestigationViewLoad.Execute:- Error while Cleaning up the session objects: " + e);
		}
        //context
		...
		if(businessType!=null && businessType.equalsIgnoreCase(NEDSSConstants.SUSCEPTIBILITY_BUSINESS_OBJECT_TYPE))
		       return mapping.findForward(this.getActionForward(formCd+"_page", request));
        ...
        else
        {
            try
            {
               ...
               // ((InvestigationForm)form).getAttributeMap().put("PageNumber","2");//  ((InvestigationForm)form).getAttributeMap().get("PageNumber");
