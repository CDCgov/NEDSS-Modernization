...
public class RulesEngineUtil extends RuleConstants{
	static final LogUtils logger = new LogUtils(RulesEngineUtil.class.getName());
	 ...
	 */
	public Map<Object,Object> initiateForm(Map<Object,Object> questionMap, PamForm form) throws NEDSSAppException {
		...
		try {
			...
			if (questionMap != null && questionMap.size() > 0) {
				...
				while (ite.hasNext()) {
					...
					qMetadata = (NbsQuestionMetadata) ite
							.next();
					...
					if(answerMap.get(qMetadata.getQuestionIdentifier())==null ){
					...
					try {
						...
						logger.error("Error in initiate form ..answerArrayMap:" +e.getMessage());
						e.printStackTrace();
					}
					
					if(qMetadata.getQuestionIdentifier()!=null && qMetadata.getQuestionIdentifier().equals("TUB279")) {
						logger.debug(qMetadata);	
					}
					//Autocomplete
			...
			//For Preview, initializing Form is sufficient
			if(actionMode!=null &&  actionMode.equals(NEDSSConstants.PREVIEW_ACTION))
				return formFieldMap;
			
			TBSpecificRules tbSpec = new TBSpecificRules();
			VaricellaSpecificRules vrSpec = new VaricellaSpecificRules();
			if ((formCd != null)
					&& formFieldMap != null
					&& formFieldMap.size() > 0
					&& actionMode != null
				...
				fireRules(null,  form, formFieldMap);
				if(formCd.equals(NBSConstantUtil.INV_FORM_RVCT)){
					// commented temporarily for integrating the new the UI .
					tbSpec.fireVercritRuleOnSubmit(actionMode, formFieldMap, null,
							answerMap, answerArrayMap);
			...
			if (formFieldMap != null && formFieldMap.size() > 0
				...
				while (fIte.hasNext()) {
					...
					if (fieldId != null && ruleMap != null
						...
						fireRules((String) fieldId, form, formFieldMap);
				}
				
				if(formCd != null && formCd.equals(NBSConstantUtil.INV_FORM_VAR) && !actionMode.equals(NEDSSConstants.PREVIEW_ACTION)){
					if(actionMode!=null && actionMode.equals(NEDSSConstants.CREATE_LOAD_ACTION)){
			    	Collection<Object>  fieldCollection  = this.calculateMMWR(null, null, null, formFieldMap, formCd);
			    	if(fieldCollection!=null && fieldCollection.size()>0){
			    		Iterator<Object> ite = fieldCollection.iterator();
			    		while(ite.hasNext()){
			    			FormField fField = (FormField)ite.next();
			    			answerMap.put(fField.getFieldId(),fField.getValue());
			    		}
			    		}
			    	}
			    }
				
				// temporarily not calling vercrictRulr- Need to fix it later
				if ((formCd != null && formCd.equals(NBSConstantUtil.INV_FORM_RVCT))
						&& (actionMode.equals(NEDSSConstants.EDIT_LOAD_ACTION)
						|| actionMode.equals(NEDSSConstants.EDIT_SUBMIT_ACTION)
						|| actionMode.equals(NEDSSConstants.VIEW_LOAD_ACTION)))
					tbSpec.fireVercritRuleOnLoad(formFieldMap);
			}
		} catch (Exception e) {
			logger.fatal("RulesEngineUtil.initiateForm Exception in " , qMetadata.toString() );
			logger.fatal("RulesEngineUtil.initiateForm " , e.getMessage());
	 ...
	 * @throws NEDSSAppException 
	 */
	public Collection<Object>  fireRules(String sourceInd, PamForm form, Map<Object,Object> formFieldMap) throws NEDSSAppException {

		
		
		String formCode = form.getPamFormCd();
		String actionMode = form.getActionMode();
        ...
        if (sourceInd != null) {
			...
			// TBSpecific Rule
			if (sourceInd.equals(RuleConstants.TUB119))
				ruleColl = tbSpec.handleTUB119Rule(ruleColl,
						formFieldMap, sourceInd);
			if (ruleColl != null && ruleColl.size() > 0) {
				...
				while (ruleIter.hasNext()) {
					...
					if (rulesDT.getRuleName().equals(ENABLE)) {
						...
						this.enable(fieldColl, rulesDT, sourceInd,
								formFieldMap, ruleMap);
					} else if (rulesDT.getRuleName().equals(
							NEDSSConstants.FILTER)) {
						// call the filter rule
						this.filter(fieldColl, rulesDT, sourceInd,
								formFieldMap, answerArrayMap, formCode, form);
					} else if (rulesDT.getRuleName()
							.equals(NEDSSConstants.MMWR)) {
						// call the MMWR rule
						this.calculateMMWR(fieldColl, rulesDT, sourceInd,
								formFieldMap,formCode);
					} else if (rulesDT.getRuleName().equals(
							NEDSSConstants.DISABLE)) {
						// call the disable rule
						this.disable(fieldColl, rulesDT, sourceInd,
								formFieldMap, ruleMap);
					}
						else if(rulesDT.getRuleName().equals(
								NEDSSConstants.SINGLE_SRC_SAME_TARGET_FILTER)){
							if(!ruleFiredMap.containsKey("T"))
							 this.singleSrcSameTargetFilter(fieldColl, rulesDT, sourceInd,
									formFieldMap, answerArrayMap, formCode, form,ruleFiredMap);
						}else if(rulesDT.getRuleName().equals(
								NEDSSConstants.CASCADING_ENABLE_ON_FILTER_TARGET)){
								this.cascadingEnableOnFilterTarget(fieldColl, rulesDT, sourceInd,
									formFieldMap, answerArrayMap, formCode, form);
					}
    		...
    		if (formCode != null
					&& formCode.equals(NBSConstantUtil.INV_FORM_RVCT)) {
				// call the hard coded rules here
				tbSpec.handleRule998(fieldColl, formFieldMap);
				tbSpec.handleRule1003(fieldColl, formFieldMap, answerArrayMap);
				tbSpec.handleRule993(fieldColl, formFieldMap, answerArrayMap);
				tbSpec.handleRule1004(fieldColl, formFieldMap, answerArrayMap);
				tbSpec.handleRule989(fieldColl, formFieldMap);
			}else if (formCode != null
					&& formCode.equals(NBSConstantUtil.INV_FORM_VAR)) {
				vrSpec.handleRule1150(fieldColl, formFieldMap, answerArrayMap);
				vrSpec.handleRule1244(fieldColl, formFieldMap, answerArrayMap);
			}
		...
		logger.debug(" .........End of fireRule in RulesEngineUtil..." + end);
			...
			String message = "Exception in rulesEngineUtil while executing the rule :"+rule.toString();
			logger.fatal(message);
			ex.printStackTrace();
			throw new NEDSSAppException(message, ex);
		}
    ...
    */
	...
	public Collection<Object>  enable(Collection<Object> fieldColl, RulesDT rulesDT,
		 ...
		 */
			...
			//			for(int i=1;i<fieldColl.size();i++){
			//				fieldColl = this.getTargetsOfTargets(fieldColl, sourceInd, formFieldMap, ruleMap);
			//				break;
			//			}
			//
			//		}
	...
	public Collection<Object>  filter(Collection<Object> fieldColl, RulesDT rulesDT,  String sourceInd, Map<Object,Object> formFieldMap,  Map<Object,Object> answerArrayMap,String formCode, PamForm form){
		// get the SourceField properties from the Map
		FormField formField = (FormField) formFieldMap.get(sourceInd);
	...
	public Collection<Object>  cascadingEnableOnFilterTarget(Collection<Object> fieldColl, RulesDT rulesDT,  String sourceInd, Map<Object,Object> formFieldMap,  Map<Object,Object> answerArrayMap,String formCode, PamForm form) throws NEDSSAppException{
		// get the SourceField properties from the Map
		FormField formField = (FormField) formFieldMap.get(sourceInd);
	...
	public Collection<Object>  singleSrcSameTargetFilter(Collection<Object> fieldColl, RulesDT rulesDT,  String sourceInd, Map<Object,Object> formFieldMap,  Map<Object,Object> answerArrayMap,String formCode, PamForm form, Map<Object,Object> ruleFiredMap){
		// get the SourceField properties from the Map
		FormField formField = (FormField) formFieldMap.get(sourceInd);
		resetTargetFilters(rulesDT, formFieldMap, fieldColl);
			Collection<Object>  sourceColl = rulesDT.getSourceColl();
     ...
     * @return
     */
    public Collection<Object>  requiredForNotification(Collection<Object> fieldColl, Map<Object,Object> formFieldMap, PamForm form) {
	  ...
	  if(metContition){
		...
		if (srcFormField != null
			...
			if (tgtFormField.getValue() == null
				...
				// errorMessage =
				// targetValueDT.getErrorDescTxt().replaceAll("\\(Field
				// Name\\)",tgtFormField.getLabel());
				//
				// tgtFormField.getErrorMessage().add(errorMessage);
				// tgtFormField.setErrorStyleClass(RuleConstants.REQUIRED_FIELD_CLASS);
				//				errColl.add(tgtFormField);
 ...
 */
 public Collection<Object>  uniqueCaseNumber(Collection<Object> errColl, RulesDT rulesDT, Map<Object,Object> formFieldMap,String formCode, String actionMode, String caseLocalId){
	 ...
	 try{
		 ...
		 logger.error("Exception in the pamProxyEJB call :");
		 e.printStackTrace();
	 }
