...
public class ShareCaseSubmit
    extends Action
{

    protected static NedssUtils nedssUtils = null;
    //For logging
    static final LogUtils logger = new LogUtils(ShareCaseSubmit.class.getName());
         ...
         if (juris == null || juris.equals("")) {
           try {
             juris = (String) NBSContext.retrieve(request.getSession(),
   			...
   			logger.error("Error in ShareCaseSubmit: "+ex.getMessage());
   			ex.printStackTrace();
           }
         }
       }
       ...
       this.setTransferOwnership(obsUID,program,juris,session);
       ...
       catch (gov.cdc.nedss.exception.NEDSSConcurrentDataException e)
	     {
		  ...
		  return mapping.findForward("dataerror");
			...
			logger.error("Error in ShareCaseSub: "+ex.getMessage());
			ex.printStackTrace();
...
private Long setTransferOwnership(Long observationUID, String newProgramAreaCode,
	...
	try {
		...
		logger.error("Exception in transferOwnership: " + e.getMessage());
		e.printStackTrace();
		throw new ServletException("Error occurred in Transfer Ownership Submit : "+e.getMessage());
	}   
				}
}