...
function mathDataUpdate (operation, resultId, tableId, columnId, rowsNumber){
	var result = getElementByIdOrByName(resultId);
	newResult = 0.0;
	if  (operation=="sum"){
		for (i=0; i<rowsNumber; i++) {
			var cel = getElementByIdOrByName("mathData_t" + tableId + "l" + i +"c" + columnId);
...
function initMenu(menu) {
	if (getMenuCookie(menu)=="hide") {
		getElementByIdOrByName(menu).style.display="none";
	} else {
		getElementByIdOrByName(menu).style.display="";
	}
}

function changeMenu(menu) {
if (getElementByIdOrByName(menu).style.display=="none") {
	getElementByIdOrByName(menu).style.display="";
	element = getElementByIdOrByName(menu+"b");
	if (element != null) {
		getElementByIdOrByName(element).style.display="none";
	}
	...
	element = getElementByIdOrByName(menu+"b");
	if (element != null) {	
		var width = getElementByIdOrByName(menu).offsetWidth;	
		if (navigator.vendor == ("Netscape6") || navigator.product == ("Gecko"))
			getElementByIdOrByName(menu+"b").style.width = width;	
		else 
			getElementByIdOrByName(menu+"b").width = width;
		getElementByIdOrByName(menu+"b").style.display="";
	}
...
function changeDisplayState(elementId, imageId, visibleImageSrc, hiddenImageSrc) {
	...
	if (imageId) {
		getElementByIdOrByName(imageId).src = imgsrc + (display ? visibleImageSrc : hiddenImageSrc);
	}
}
...
function arraySort(tableName, column, lineNumber, columnNumber) {
	...
	if (aTable) {
		...
		for (i=0;i<lineNumber;i++) {	
			...
			for (j=0;j<columnNumber;j++) {
				obj = getElementByIdOrByName("t" + tableName + "l" + (i+1) +"c" + j);		
	...
	for (i=0;i<lineNumber;i++) {
		...
		for (j=0;j<columnNumber;j++) {
			getElementByIdOrByName("t" + tableName + "l" + (i+1) +"c" + j).innerHTML = array[goodLine][j];
		}
		getElementByIdOrByName("t" + tableName + "l" + (i+1) +"c" + 0).parentNode.parentNode.onmouseover = array[goodLine][columnNumber];
		getElementByIdOrByName("t" + tableName + "l" + (i+1) +"c" + 0).parentNode.parentNode.onmouseout = array[goodLine][columnNumber+1];
	}
}
...
//	document.write('<form>');
	document.write('<tr><td colspan="15" class="CALENDARBORDER"><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=1></td></tr>');
	document.write('<tr>');
	document.write('	<td class="CALENDARBORDER" width="1"><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=20></td>');
	document.write('	<td class="CALENDARTITLE" colspan="3" align="right"><img src="' + imgsrc + 'previous.gif" alt ="Previous" title = "Previous" onclick="cal_before(' + day + ');"></td>');
	...
	document.write('	</td>');
	document.write('	<td class="CALENDARTITLE" align="left" colspan="3"><img src="' + imgsrc + 'next.gif" alt="Next" title="Next" onclick="cal_after(' + day + ');">&nbsp;&nbsp;<img src="' + imgsrc + 'close.gif" alt = "Close" title = "Close" onclick="hideCalendar()"></td>');
	document.write('	<td class="CALENDARBORDER" width=1><img src="' + imgsrc + 'shim.gif" alt = "" width="1" height="1"></td>');
	document.write('</tr>');
	document.write('<tr><td colspan=15 class="CALENDARBORDER"><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=1></td></tr>');
	document.write('<tr>');
	document.write('	<td class="CALENDARBORDER" width="1"><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=1></td>');
	document.write('	<td class="CALENDRIER" width="38">' + computedcaldays[0] + '</td>');
	document.write('	<td class="CALENDRIER" width="1"><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=1></td>');
...
function showCalendar(year, month, day, pattern, formName, formProperty, event, startYear, endYear) {
	...
	if (startYear!=null) {
		var calyear = getElementByIdOrByName("calyear");
	...
	if (document.layers) {
		...
		getElementByIdOrByName("calmois").selectedIndex=month;
	}
	...
	if(document.all) {
		...
		// Mozilla
		var calendrier = getElementByIdOrByName("slcalcod");
		var position = cal_place(event);
...
function cal_chg(day, month, year){
	...
	champMonth = getElementByIdOrByName("calmois");
	if (month==null) {		
	...
	champYear = getElementByIdOrByName("calyear");
	if (year==null) {		
	...
	if(month>0) {
		...
		for(u=0;u<6;u++){
			...
			for(i=0;i<7;i++){
				...
				str+='		<td class="CALENDARBORDER" width=1><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=20></td>\n';
			...
			str+='		<td class="CALENDARBORDER" width=1><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=1></td>\n';
			str+='	</tr>\n';
			str+='	<tr><td colspan=15 class="CALENDARBORDER"><img src="' + imgsrc + 'shim.gif" alt = "" width=1 height=1></td></tr>\n';
		}
		str+='</table>\n';
	...
	if (!document.all && getElementByIdOrByName) {
		getElementByIdOrByName("caljour").innerHTML = str;
	}
}
 ...
 */
function selectTab(tabGroupId, tabGroupSize, selectedTabId, enabledStyle, disabledStyle, errorStyle, tabKeyName,
    tabKeyValue)
{ 
  // first unselect all tab in the tag groups.
  JBaseForm.setTabId(selectedTabId);
  for (i = 0; i < tabGroupSize; i++) {
    element = $j("[id=" + ("tabs" + tabGroupId + "head" + i) + "]"); 
    for (j = 0; j < element.length; j++) { 
      if (element[j] == null)
        continue;
      if (element[j].classNameErrorStdLayout) {
        element[j].className = errorStyle;
        element[j].style.color = "";
      } else if (element[j].className == enabledStyle) {
        if(element[0] != null)
   		  element[0].className = "ongletTextDis";
		if( element[1] != null )
           element[1].className = "ongletTextDis1";
        element[j].style.color = "";
      } else if (element[j].className == errorStyle) {
        // do nothing more
      } 
    }  
    $j("#" + "tabs" + tabGroupId + "tab" + i).hide(); 
    javascript: scroll(0, 0);
  }
  element = $j("[id=" + "tabs" + tabGroupId + "head" + selectedTabId  + "]");   

  for (k = 0; k < element.length; k++) { 
    if (element[k].className == errorStyle) {
      element[k].classNameErrorStdLayout = new Object();
    }
	if(element[0] != null)
      element[0].className = "ongletTextEna";
	if(element[1] != null){
	      element[1].className = "ongletTextEna1";
		  element[1].parentNode.className="tablecell";
		}
    element[k].style.cursor = "default"; 
  } 
  $j("#tabs" + tabGroupId + "tab" + selectedTabId).show();
  ...
  // defined in the StrutsLayoutTabControl.js file
...
function onTabHeaderOver(tabGroupId, selectedTabId, enabledStyle)
{
  ...
  for (i = 0; i < element.length; i++) {
    if (element[i].className == enabledStyle) {
...
function loadTree(url, tree) {
	element = getElementByIdOrByName("treeView" + url);
	element.innerHTML = tree;	
	element.style.display = "";
	element = getElementByIdOrByName("treeViewNode" + url);
...
function changeTree(tree, image1, image2) {
	if (!isTreeviewLocked(tree)) {
		var image = getElementByIdOrByName("treeViewImage" + tree);
		...
		if (getElementByIdOrByName("treeView" + tree).innerHTML == "") {
			return true;
		} else {
		...
		for (i=0; i<list.length; i++) {
			...
			if (currentElement.id.indexOf(menu) != -1
				...
				if (currentElement.style.display=="none" && toShow
					...
					image = getElementByIdOrByName("treeViewImage" + subTreeName);
					...
					if (link.href.indexOf("javascript://") != -1) {
						...
						if (getElementByIdOrByName("treeView" + subTreeName).innerHTML == "") {
							//return true;
						} else {
...
function lockTreeview(in_name) {
	...
	var item = getElementByIdOrByName("treeView" + in_name);
...
function showLayoutLayer(ID,event) {
	var thisLayer = getElementByIdOrByName(ID);
	objLeft = 0;
	objTop = 0;
	...
	var layerPosX = event.clientX - objLeft;	
   	...
   	getElementByIdOrByName(ID).style.top = layerPosY + layerYOffset;
   	getElementByIdOrByName(ID).style.left = finalPosX;
//	document.getElementById(ID).style.zIndex= 3;
   	getElementByIdOrByName(ID).style.visibility = "visible";
}

function hideLayoutLayer(ID) {
  getElementByIdOrByName(ID).style.visibility = "hidden";
}
