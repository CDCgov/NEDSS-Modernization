...
/**
 * Updated version of <code>ResultSetUtils</code> that supports
 * floating-point numbers
 * 
 * @author rdodge
 */
public class GeoCodingResultSetUtils {

	static final LogUtils logger = new LogUtils(GeoCodingResultSetUtils.class.getName());
	...
	/**
	 * Maps the data returned in a ResultSet to a List
	 *
	 * @param resultSet
	 *            the result set
	 * @param metaData
			...
			for (int i = 1; i <= columns; i++) {
				...
				if (columnValue != null) {
					...
					if (parmMethodMap == null) {
						logger.debug("Error: no method in " + beanClass
								+ " with name " + methodName);
					} else {
						Method method = getMethodForClass(parmMethodMap, columnValueClass);
																			...
																			// SQL Server
						{
							...
							method = getMethodForClass(parmMethodMap, columnValueClass);
						} // if

						if (method == null) {
							logger.error("Error: no method in " + beanClass
							...
							try {
								...
								logger.debug("IllegalArgumentException thrown.");
								// ie.printStackTrace();
								Object val = null;
								// for
								// SQL Server
								...
								try {
									...
									logger.error("Illegal argument in resultset.");
								} // catch
							} // catch
						} // else
					}// else
				...
				for (int i = 0; i < methods.length; i++) {
					...
					if (methodName.startsWith("set") && parmTypes.length == 1) {
						...
						if (parmTypes[0].equals(Boolean.class)
							...
							if (verbose) {
								logger.debug("Adding " + methodName + " with parameter type String");
							}
						}

						/*
						 * Add BigDecimal mapping for Long, Integer, Float and Double types
						 */
						if (parmTypes[0].equals(Long.class)
								|| parmTypes[0].equals(Integer.class)
								|| parmTypes[0].equals(Float.class)
								|| parmTypes[0].equals(Double.class)) {

							parmMethodMap.put(java.math.BigDecimal.class, method);
							if (verbose) {
								logger.debug("Adding " + methodName + " with parameter type BigDecimal");
							}
						}
					}
				}
	...
	public static void main(String[] args) {
		 ...
		 * rsu = new ResultSetUtils(); Map<Object, Object><Object,Object> rmap = null; rmap =
