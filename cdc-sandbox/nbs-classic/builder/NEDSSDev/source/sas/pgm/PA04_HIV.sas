/*Program Name : PA04_HIV.sas																											*/
/*																																				*/
/*Original Program Created by : SA																												*/
/*																																				*/
/*Program Created Date:	11-21-2016																												*/
/*																																				*/
/*Program Last Modified Date:12-15-2016																											*/
/*							:01-04-2017	- FORMATTING COMPLETE																												*/
/*							:																													*/
/*Program Description:	Creates PA04 Program Indicator HIV report for NBS5.1 																														*/
/*																																				*/
/*Comments:																																		*/
...
and a.CC_CLOSED_DT~=. and a.CA_PATIENT_INTV_STATUS in ('I - Interviewed');
quit;
...
if find(descrip,'Period partner index:','i') ge 1 then value1 = ROUND("&Val_K",0.1) ;
...
inner join nbs_rdb.investigation e on e.investigation_key =a.investigation_KEY
inner join nbs_rdb.d_contact_record on d_contact_record.d_contact_record_key = g.d_contact_record_key
...
create table pix as 
select  
count(distinct a.inv_local_id) as var_n, c.IX_TYPE 
from STD_HIV_DATAMART1 a
inner join nbs_rdb.investigation e  on e.investigation_key = a.investigation_KEY
...
left outer join nbs_rdb.D_INTERVIEW c on  c.D_INTERVIEW_KEY = d.CONTACT_INTERVIEW_KEY and c.RECORD_STATUS_CD~='LOG_DEL'
left outer join nbs_rdb.D_provider d  on d.provider_key = a.INVESTIGATOR_INTERVIEW_KEY
...
select  count(distinct f.inv_local_id) as var_o ,  c.IX_TYPE 
from STD_HIV_DATAMART1 a
inner join nbs_rdb.investigation e  on e.investigation_key = a.investigation_KEY
inner join nbs_rdb.F_CONTACT_RECORD_CASE d on a.INVESTIGATION_KEY = d.SUBJECT_INVESTIGATION_KEY
inner join nbs_rdb.STD_HIV_DATAMART f on d.CONTACT_INVESTIGATION_KEY = f.Investigation_key
...
/*********** Partner Notification Index*****/
proc sql;
create table pix1 as 
select  
count(distinct a.inv_local_id) as var_nc, c.IX_TYPE 
from STD_HIV_DATAMART1 a
inner join nbs_rdb.investigation e  on e.investigation_key = a.investigation_KEY
inner join nbs_rdb.F_CONTACT_RECORD_CASE d on a.INVESTIGATION_KEY = d.SUBJECT_INVESTIGATION_KEY
inner join nbs_rdb.STD_HIV_DATAMART f on d.CONTACT_INVESTIGATION_KEY = f.Investigation_key
inner join nbs_rdb.D_CONTACT_RECORD g on g.D_CONTACT_RECORD_KEY = d.D_CONTACT_RECORD_KEY and g.RECORD_STATUS_CD~='LOG_DEL'
left outer join nbs_rdb.D_INTERVIEW c on  c.D_INTERVIEW_KEY = d.CONTACT_INTERVIEW_KEY and c.RECORD_STATUS_CD~='LOG_DEL'
left outer join nbs_rdb.D_provider h  on h.provider_key = a.INVESTIGATOR_INTERVIEW_KEY
...
proc sql;   /***********Testing Index for HIV*****/
create table testindex1 as 
select  count(distinct f.inv_local_id) as var_oc ,  c.IX_TYPE 
from STD_HIV_DATAMART1 a
inner join nbs_rdb.investigation e  on e.investigation_key = a.investigation_KEY
inner join nbs_rdb.F_CONTACT_RECORD_CASE d on a.INVESTIGATION_KEY = d.SUBJECT_INVESTIGATION_KEY
inner join nbs_rdb.STD_HIV_DATAMART f on d.CONTACT_INVESTIGATION_KEY = f.Investigation_key
...
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION 
in ('1 - Prev. Pos' , '2 - Prev. Neg, New Pos' , 
...
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('1 - Prev. Pos');

select count(distinct inv_local_id) into :Val_PB 
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('2 - Prev. Neg, New Pos');

select count(distinct inv_local_id) into :Val_PC 
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('3 - Prev. Neg, Still Neg');

select count(distinct inv_local_id) into :Val_Pd
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('4 - Prev. Neg, No Test');

select count(distinct inv_local_id) into :Val_Pe
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('5 - No Prev Test, New Pos');

select count(distinct inv_local_id) into :Val_Pf
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('6 - No Prev Test, New Neg');

select count(distinct inv_local_id) into :Val_P7
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and FL_FUP_DISPOSITION in ('7 - No Prev Test, No Test');

select count(distinct inv_local_id) into :Val_Pq
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and 
FL_FUP_DISPOSITION in ('G - Insufficient Info to Begin Investigation',
'H - Unable to Locate',
...
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original') and 
FL_FUP_DISPOSITION in ('1 - Prev. Pos' , '2 - Prev. Neg, New Pos' , 
'3 - Prev. Neg, Still Neg' , 
'4 - Prev. Neg, No Test' ,'5 - No Prev Test, New Pos' , 
'6 - No Prev Test, New Neg' , '7 - No Prev Test, No Test');
...
select sum(var_nc) into :Val_n1
from pix1 
WHERE IX_TYPE IN ('Initial/Original');

select sum(var_oc)  into :Val_o1 			 
from testindex1 
WHERE IX_TYPE IN ('Initial/Original');
quit;
...
%let PER_P7 = %sysevalf(%eval(&Val_P7) / %eval(&VAL_PN)) ;
...
%let PER_C7 = %sysevalf(%eval(&Val_C7) / %eval(&VAL_CN)) ;
...
%let VAL_PO = %sysevalf(%eval(&VAL_N) / %eval(&Val_B)) ;
%let VAL_PP = %sysevalf(%eval(&VAL_O) / %eval(&Val_B)) ;



%let VAL_CO = %sysevalf(%eval(&VAL_N1) / %eval(&Val_B)) ;
%let VAL_CP = %sysevalf(%eval(&VAL_O1) / %eval(&Val_B)) ;
...
set templatePA04_HIV;
if find(descrip,'PARTNERS INITIATED:','i') ge 1 then &name = "&val_PM" ;
if find(descrip,'PARTNERS EXAMINED:','i') ge 1 then &name = "&val_PN" ;
if find(descrip,'DISPO. 1 - PREV POS:','i') ge 1 then &name = "&val_PA" ;
if find(descrip,'DISPO. 2 - PREV NEG, NEW POS:','i') ge 1 then &name = "&val_PB" ;
if find(descrip,'NOTIF. INDEX.:','i') ge 1 then &name = ROUND("&val_PO",0.01);
if find(descrip,'TESTING. INDEX.:','i') ge 1 then &name = ROUND("&val_PP",0.01);
if find(descrip,'DISPO. 3 - PREV NEG, STILL NEG:','i') ge 1 then &name = "&val_PC";
if find(descrip,'DISPO. 4 - PREV NEG, NO TST:','i') ge 1 then &name = "&val_PD";

if find(descrip,'DISPO. 5 - NO PREV TST, NEW POS:','i') ge 1 then &name = "&val_PE" ;
if find(descrip,'DISPO. 6 - NO PREV TST, NEW NEG:','i') ge 1 then &name = "&val_PF" ;
if find(descrip,'DISPO. 7 - NO PREV TST, NO TST:','i') ge 1 then &name = "&val_P7" ;
...
if find(descrip,'DISPO. 1 - PREV POS:','i') ge 1 then &name = "&val_CA" ;
if find(descrip,'DISPO. 2 - PREV NEG, NEW POS:','i') ge 1 then &name = "&val_CB" ;
if find(descrip,'NOTIF. INDEX:','i') ge 1 then &name = ROUND("&val_CO",0.01);
if find(descrip,'TESTING. INDEX:','i') ge 1 then &name = ROUND("&val_CP",0.01);
if find(descrip,'DISPO. 3 - PREV NEG, STILL NEG:','i') ge 1 then &name = "&val_CC";
if find(descrip,'DISPO. 4 - PREV NEG, NO TST:','i') ge 1 then &name = "&val_CD";

if find(descrip,'DISPO. 5 - NO PREV TST, NEW POS:','i') ge 1 then &name = "&val_CE" ;
if find(descrip,'DISPO. 6 - NO PREV TST, NEW NEG:','i') ge 1 then &name = "&val_CF" ;
if find(descrip,'DISPO. 7 - NO PREV TST, NO TST:','i') ge 1 then &name = "&val_C7" ;
...
if find(descrip,'DISPO. 1 - PREV POS:','i') ge 1 then &per = put(round("&per_PA",0.001),percent8.1) ;
if find(descrip,'DISPO. 3 - PREV NEG, STILL NEG:','i') ge 1 then &per = put(round("&per_PC",0.001),percent8.1) ;

if find(descrip,'DISPO. 2 - PREV NEG, NEW POS:','i') ge 1 then &per = put(round("&per_PB",0.001),percent8.1) ;
if find(descrip,'DISPO. 4 - PREV NEG, NO TST:','i') ge 1 then &per = put(round("&per_PD",0.001),percent8.1) ;
if find(descrip,'DISPO. 5 - NO PREV TST, NEW POS:','i') ge 1 then &per = put(round("&per_PE",0.001),percent8.1) ;
if find(descrip,'DISPO. 6 - NO PREV TST, NEW NEG:','i') ge 1 then &per = put(round("&per_PF",0.001),percent8.1) ;
if find(descrip,'PARTNERS NOT EXAMINED:','i') ge 1 then &per = put(round("&per_PQ",0.001),percent8.1) ;
if find(descrip,'DISPO. 7 - NO PREV TST, NO TST:','i') ge 1 then &per = put(round("&per_P7",0.001),percent8.1) ;
...
if find(descrip,'DISPO. 1 - PREV POS:','i') ge 1 then &per = put(round("&per_CA",0.001),percent8.1) ;
if find(descrip,'DISPO. 3 - PREV NEG, STILL NEG:','i') ge 1 then &per = put(round("&per_CC",0.001),percent8.1) ;

if find(descrip,'DISPO. 2 - PREV NEG, NEW POS:','i') ge 1 then &per = put(round("&per_CB",0.001),percent8.1) ;
if find(descrip,'DISPO. 4 - PREV NEG, NO TST:','i') ge 1 then &per = put(round("&per_CD",0.001),percent8.1) ;
if find(descrip,'DISPO. 5 - NO PREV TST, NEW POS:','i') ge 1 then &per = put(round("&per_CE",0.001),percent8.1) ;
if find(descrip,'DISPO. 6 - NO PREV TST, NEW NEG:','i') ge 1 then &per = put(round("&per_CF",0.001),percent8.1) ;
if find(descrip,'DISPO. 7 - NO PREV TST, NO TST:','i') ge 1 then &per = put(round("&per_C7",0.001),percent8.1) ;
...
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION 
in ('1 - Prev. Pos' , '2 - Prev. Neg, New Pos' , 
'3 - Prev. Neg, Still Neg' , 
'4 - Prev. Neg, No Test' ,'5 - No Prev Test, New Pos' , 
'6 - No Prev Test, New Neg' , '7 - No Prev Test, No Test');
...
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('1 - Prev. Pos');

select count(distinct inv_local_id) into :Val_PB 
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('2 - Prev. Neg, New Pos');

select count(distinct inv_local_id) into :Val_PC 
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('3 - Prev. Neg, Still Neg');

select count(distinct inv_local_id) into :Val_Pd
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('4 - Prev. Neg, No Test');

select count(distinct inv_local_id) into :Val_Pe
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('5 - No Prev Test, New Pos');

select count(distinct inv_local_id) into :Val_Pf
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('6 - No Prev Test, New Neg');

select count(distinct inv_local_id) into :Val_P7
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and FL_FUP_DISPOSITION in ('7 - No Prev Test, No Test');

select count(distinct inv_local_id) into :Val_Pq
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Re-Interview') and 
FL_FUP_DISPOSITION in ('G - Insufficient Info to Begin Investigation',
'H - Unable to Locate',
'J - Located, Not Examined and/or Interviewed',
'K - Sent Out Of Jurisdiction',
...
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION 
in ('1 - Prev. Pos' , '2 - Prev. Neg, New Pos' , 
'3 - Prev. Neg, Still Neg' , 
'4 - Prev. Neg, No Test' ,'5 - No Prev Test, New Pos' , 
'6 - No Prev Test, New Neg' , '7 - No Prev Test, No Test');
...
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('1 - Prev. Pos');

select count(distinct inv_local_id) into :Val_PB 
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('2 - Prev. Neg, New Pos');

select count(distinct inv_local_id) into :Val_PC 
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('3 - Prev. Neg, Still Neg');

select count(distinct inv_local_id) into :Val_Pd
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('4 - Prev. Neg, No Test');

select count(distinct inv_local_id) into :Val_Pe
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('5 - No Prev Test, New Pos');

select count(distinct inv_local_id) into :Val_Pf
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('6 - No Prev Test, New Neg');

select count(distinct inv_local_id) into :Val_P7
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and FL_FUP_DISPOSITION in ('7 - No Prev Test, No Test');

select count(distinct inv_local_id) into :Val_Pq
from PP04_OI
WHERE CC_CLOSED_DT is not null and CA_PATIENT_INTV_STATUS in ('I - Interviewed')
and IX_TYPE IN ('Initial/Original','Re-Interview') and 
FL_FUP_DISPOSITION in ('G - Insufficient Info to Begin Investigation',
'H - Unable to Locate',
'J - Located, Not Examined and/or Interviewed',
'K - Sent Out Of Jurisdiction',
...
ods text = "^{style[font_face='calibri' fontsize=14pt just=CENTER  fontweight=bold] 
						 %upcase(&reportTitle)}";
ods region x = 0.75 in y = 0.75 in width = 5.0in ;
...
ods region x = 0.00 in y = 2.65 in ;
ods text = "^{style[font_face='calibri' fontsize=11pt just=CENTER fontweight=bold] 
						FROM OI 							}";
ods region x = 2.00 in y = 2.65 in ;
ods text = "^{style[font_face='calibri' fontsize=11pt just=CENTER fontweight=bold] 
								FROM RI 		}";
ods region x = 4.25 in y = 2.65 in ;
ods text = "^{style[font_face='calibri' fontsize=11pt just=CENTER fontweight=bold] 
								TOTAL}";
ods region x = 0.25 in y = 2.75 in width = 10.75in ;
proc report data = templatePA04_HIV nowd spanrows  split='~' headline missing 
style(header)={just=left font_weight=bold font_face="calibri"
font_size = 11pt }
style(report)={font_face="calibri" font_size=10pt  rules=none frame=void vjust = middle }
style(column)={font_face="calibri" font_size=10pt vjust = middle };
...
define descrip /ORDER=internal ' '  left WIDTH=20 style(column)={asis=on cellwidth=60mm rules=none CELLHEIGHT = 4mm  }
...
if find(descrip,'PARTNERS INITIATED:','i') ge 1  then do;
  ...
  if find(descrip,'DISPO. 1 - PREV POS:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
   if find(descrip,'DISPO. 2 - PREV NEG, NEW POS:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
   if find(descrip,'DISPO. 3 - PREV NEG, STILL NEG:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
   if find(descrip,'DISPO. 4 - PREV NEG, NO TST:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
   if find(descrip,'DISPO. 5 - NO PREV TST, NEW POS:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
   if find(descrip,'DISPO. 6 - NO PREV TST, NEW NEG:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
    if find(descrip,'DISPO. 7 - NO PREV TST, NO TST:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
 ...
 if find(descrip,'DISPO. X - DECEASED:','i') ge 1  then do;
   ...
   if find(descrip,'DISPO. 1','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
  end;
   if find(descrip,'DISPO. 2 - PREV NEG, NEW POS:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.25in}');
 ...
 if find(descrip,'DISPO. X - DECEASED:','i') ge 1  then do;
   ...
   if find(descrip,'NOTIF. INDEX.:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.40in}');
  end;
   if find(descrip,'TESTING. INDEX.:','i') ge 1  then do;
    call define(_col_,'style','style={leftmargin=.40in}');
  end;
