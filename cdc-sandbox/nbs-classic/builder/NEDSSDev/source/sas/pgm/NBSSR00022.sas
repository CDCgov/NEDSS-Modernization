...
%if  %upcase(&skip)=YES %then
      %goto finish;
PROC SORT DATA = INTERVIEWER_ASSIGN_DT_DATAMART;
by CA_INTERVIEWER_ASSIGN_DT;




data INTERVIEWER_ASSIGN_DT_DATAMART;
set INTERVIEWER_ASSIGN_DT_DATAMART;
LENGTH DX $78;
...
drop CA_INTERVIEWER_ASSIGN_DT DIAGNOSIS INVESTIGATOR_NAME INVESTIGATOR_QUICK_CODE DIAGNOSIS CC_CLOSED_DT CC_CLOSED_DT;
run;
...
%footnote;
Title;
Title1 BOLD HEIGHT=5 FONT=Times J=C <p align="center"> "QA01 Interview Record Listing (OI Date)" </p>;
...
PROC REPORT DATA = INTERVIEWER_ASSIGN_DT_DATAMART NOWINDOWS; 
...
%end;
%else 
	%export(work,INTERVIEWER_ASSIGN_DT_DATAMART,sock,&exporttype);
Title;
%finish:
%mend NBSSR00022;
%NBSSR00022;
