...
/*							:01-09-2017: All upper case text for report title																	*/
...
where 
/* b.INV_CASE_STATUS in ('Probable','Confirmed') and --Defect ND-11097*/
a.INVESTIGATOR_FL_FUP_KEY ~=1 and  datepart(FL_FUP_DISPO_DT) ge datepart(FL_FUP_INVESTIGATOR_ASSGN_DT)
...
select PROVIDER_QUICK_CODE,  INVESTIGATOR_FL_FUP_KEY , count (distinct inv_local_id) as var_ac_p  
from &in
...
%include NBSPGMp1 (template8.sas);
...
compute descrip ;
 ...
 if find(descrip,'Exam''d:','i') ge 1  then do;
   ...
   if find(descrip,'Dispo E:','i') ge 1  then do;
    call define(_col_,'style','style={fontweight=bold font_size=10pt font_face="calibri"}');
  end;
...
if find(colname,'Assigned:','i') ge 1  then do;
 ...
 if find(colname,'Exam''d:','i') ge 1  then do;
   ...
   if find(colname,'Dispo E:','i') ge 1  then do;
    call define(_col_,'style','style={fontweight=bold font_size=10pt font_face="calibri"}');
  end;
