...
create table QA07_INIT as
	...
	PROVIDER_QUICK_CODE, a.DIAGNOSIS,
	...
	order by PATIENT_NAME, DIAGNOSIS
;
quit;
...
DATA QA07_DT_CALC;
	...
	DROP SPECIMEN_COLLECTION_DT DIAGNOSIS_DT INV_LOCAL_ID REFERRAL_BASIS PROVIDER_QUICK_CODE;
run;
...
DATA QA07_MERGE;
	MERGE QA07_DUP QA07_COUNT;
	by PATIENT_NAME DIAGNOSIS;
run;

proc sql;
create table QA07 as
	select * from QA07_MERGE where count >1 AND DAYS <=60
	order by PATIENT_NAME, DIAGNOSIS, FL_FUP_EXAM_DT;
quit;
