...
public class ExactMatchService {
  ...
  private static final String FIND_PIVOTS = """
      ...
      FROM   (SELECT p.person_uid               "personUid",
              ...
              WHERE  p.person_uid = pn.person_uid
                     ...
                     AND Rtrim(Ltrim(pn.first_nm)) = Rtrim(Ltrim(p2.first_nm))
  ...
  private static final String FIND_MATCHING_NAME =
      """
               ...
               DECLARE @QueryString VARCHAR(MAX);
               SET @QueryString = 'SELECT pn1.person_uid "personUid", pn1.AS_OF_DATE "asOfDate"
  ...
  private static final String FIND_MATCHING_LOCATORS =
      """
                  ...
                  DECLARE @QueryString VARCHAR(MAX);
                  SET @QueryString = 'SELECT elp1.entity_uid "personUid",
  ...
  public ExactMatchResponse match() {
    ...
    List<String> matches = findPivots();

    // verify matches all have matching person data 
    if (!matches.isEmpty()) {
      List<String> hasMatchingPersonData = findMatchesOnPersonData(matches);
      // Filter out any records that are not also person data matches
      matches = matches.stream()
          .filter(hasMatchingPersonData::contains)
          .toList();
    }

    // verify matches all have matching race data
    if (!matches.isEmpty()) {
      List<String> hasMatchingRaceData = findMatchesOnRaceData(matches);
      // Filter out any records that are not also race data matches
      matches = matches.stream()
          .filter(hasMatchingRaceData::contains)
          .toList();
    }

    // verify matches all have matching identification data
    if (!matches.isEmpty()) {
      List<String> hasMatchingIdentificationData = findMatchesOnIdentificationData(matches);
      // Filter out any records that are not also identification data matches
      matches = matches.stream()
          .filter(hasMatchingIdentificationData::contains)
          .toList();
    }

    // verify matches all have matching name data
    if (!matches.isEmpty()) {
      List<String> hasMatchingNameData = findMatchesOnNameData(matches);
      // Filter out any records that are not also name data matches
      matches = matches.stream()
          .filter(hasMatchingNameData::contains)
          .toList();
    }

    // verify matches all have matching locator data
    if (!matches.isEmpty()) {
      List<String> hasMatchingLocatorData = findMatchesOnLocatorData(matches);
      // Filter out any records that are not also locator data matches
      matches = matches.stream()
          .filter(hasMatchingLocatorData::contains)
          .toList();
    }
  ...
  private List<String> findMatchesOnPersonData(List<String> personIds) {
    // for each person id, fetch the list of matching person records and then combine into a single list.
    return personIds.stream()
        .map(this::queryMatchingPersons)
        .flatMap(List::stream)
        .toList();
  }
  ...
  private List<String> findMatchesOnRaceData(List<String> personIds) {
    // for each person id, fetch the list of matching person records and then combine into a single list.
    return personIds.stream()
        .map(id -> queryMatchingRace(id, personIds))
        .flatMap(List::stream)
        .toList();
  }
  ...
  private List<String> findMatchesOnIdentificationData(List<String> personIds) {
    // for each person id, fetch the list of matching person records and then combine into a single list.
    return personIds.stream()
        .map(id -> queryMatchingIdentification(id, personIds))
        .flatMap(List::stream)
        .toList();
  }
  ...
  private List<String> findMatchesOnNameData(List<String> personIds) {
    // for each person id, fetch the list of matching person records and then combine into a single list.
    return personIds.stream()
        .map(id -> queryMatchingNames(id, personIds))
        .flatMap(List::stream)
        .toList();
  }
  ...
  private List<String> findMatchesOnLocatorData(List<String> personIds) {
    // for each person id, fetch the list of matching person records and then combine into a single list.
    return personIds.stream()
        .map(id -> queryMatchingLocator(id, personIds))
        .flatMap(List::stream)
        .toList();
  }
