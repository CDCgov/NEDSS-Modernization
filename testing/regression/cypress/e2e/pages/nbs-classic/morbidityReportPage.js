class MorbidityReportPage {
    ...
    treatmentDateField =
        '#treatment_s\\[i\\]\\.treatmentVO_s\\[0\\]\\.treatmentAdministeredDT_s\\[0\\]\\.effectiveFromTime_s';
    ...
    errorMessages = 'font.boldTenRed, font[color="red"], .error, .errorMessage, [class*="error"]';
    ...
    clickMarkAsReviewedAndHandlePopup() {
        ...
        cy.get('#Mark\\ as\\ Reviewed').should('be.visible').first().click();
    }
    ...
    verifyFieldValidationError(fieldName) {
        ...
        cy.get(this.errorMessages).filter(':visible').should('have.length.greaterThan', 0);
    }
    ...
    verifySuccessfulSubmission() {
        ...
        cy.url().should('satisfy', (url) => {
            return url.includes('ObservationMorbDataEntry1') || url.includes('MyTaskList1') || url.includes('Home');
        });
