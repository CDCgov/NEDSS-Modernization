class GroupAndUngroupQuestions {
    ...
    checkForGroupedQuestions() {
        cy.get('body').then((ele) => {
            if (ele.find('[data-testid="questionGroupIndicator"]').length > 0) {
                ...
                cy.get('.subsectionOptionsWithGrouped-')
                    .eq(0)
                    .get('[data-testid="groupQuestionsOption"]')
                    .eq(0)
                    .click();
                this.renameBlockName();
                this.fillTableColumnWidths();
                cy.get('[data-testid="group-questions-submit-btn"]').filter(':visible').eq(0).click();
                cy.wait(2000);
                cy.get('[data-testid="questionGroupIndicator"]').eq(0);
            }
        });
    ...
    clickUngroupQuestionOption() {
        cy.get('.subsectionOptionsWithGrouped-grouped')
            .eq(0)
            .get('[data-testid="ungroupQuestionsOption"]')
            .eq(0)
            .click();
    }
    ...
    clickEditSubsectionOption() {
        cy.get('.subsectionOptionsWithGrouped-grouped').eq(0).get('[data-testid="editSubsectionOption"]').eq(0).click();
    }
    ...
    clickGroupedSubmitBtn() {
        cy.get('[data-testid="subsection-submit-btn"]').filter(':visible').eq(0).click();
    }
    ...
    clickGroupQuestionOption() {
        cy.wait(2000);
        cy.get('.subsectionOptionsWithGrouped-ungrouped')
            .eq(1)
            .get('[data-testid="groupQuestionsOption"]')
            .eq(0)
            .click();
