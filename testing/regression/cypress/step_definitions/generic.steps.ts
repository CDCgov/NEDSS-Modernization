...
When('I navigate to {string} path', (url: string) => {
    cy.visit(url);
});

// use for NBS 6 input buttons
When('I click the {string} input button', (inputName: string) => {
...
Then('I should see the {string} link', (name: string) => {
    cy.contains('a', name).should('be.visible');
});

When('I select value {string} in the {string} field', (value: string, label: string) => {
    cy.findByRole('combobox', { name: label }).select(value);
});

When('I toggle the {string} field', (label: string) => {
...
When('I select radio {string} in the {string} field', (value: string, label: string) => {
...
Then('I should see a {string} labelled {string}', (role: string, name: string) => {
    // the dialog itself is 0x0, so not exactly "visible", but it's there
    cy.findByRole(role, { name }).should('be.visible');
});

Then('I should see value {string} in the {string} field', (value: string, label: string) => {
    cy.findByRole('definition', { name: label }).should('have.text', value);
});

Then('I should see {string} validation error', (errorMsg: string) => {
...
Then(
    'I should see value {string} in the {string} {string} input field',
    (value: string, label: string, role: string) => {
        cy.findByRole(role, { name: label }).should('have.value', value);
    }
);

Then('I should see option {string} in the {string} combobox input field', (value: string, label: string) => {
    cy.findByRole('combobox', { name: label }).find('option:selected').should('have.text', value);
});

Then(
    'I should see option {string} in the {string} multi-select combobox input field',
...
Then('I am redirected to {string}', (pathname: string) => {
...
Then('I should see a green success message containing text {string}', (text: string) => {
