...
Given(
    ...
    (
        ...
        cy.readFile(`cypress/fixtures/${fixtureName}.json`, 'utf8').then((file) => {
...
Then('the Investigation has a notification with a status of {string}', (exectedStatus) => {
    expect(investigationId, 'Investigation Id').is.not.null;

    const transportstatusurl = Cypress.env('NOTIFICATION_STATUS_API').replace('uid', investigationId);
...
Then('the Notification is copied onto the on-prem database with a status of {string}', (exectedStatus) => {
    expect(notificationId, 'Investigation Id').is.not.null;

    const transportstatusurl = Cypress.env('ON_PRIM_NOTIFICATION_STATUS_API').replace('uid', notificationId);
    ...
    const checkStatus = (attempts = 0) => {
        ...
        cy.request({
            ...
            status = response.body.transportStatus ? response.body.transportStatus : response.body.netssTransportStatus;
            cy.log('Recieved status of: ' + status);
            if (status === exectedStatus) {
                return;
            } else {
